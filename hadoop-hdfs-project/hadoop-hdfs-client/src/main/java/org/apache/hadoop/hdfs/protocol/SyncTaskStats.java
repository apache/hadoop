...
public class SyncTaskStats {
  ...
  @VisibleForTesting
  SyncTaskStats(boolean success, MetadataSyncTaskOperation operation, long numberOfBytes) {
    assert success;
  ...
  public static SyncTaskStats from(MetadataSyncTaskExecutionFeedback feedback){
    if(SyncTaskExecutionOutcome.FINISHED_SUCCESSFULLY == feedback.getOutcome()){
      return new SyncTaskStats(true, feedback.getOperation(), feedback.getResult().getNumberOfBytes());
    } else {
  ...
  public static SyncTaskStats from(BlockSyncTaskExecutionFeedback feedback){
    if(SyncTaskExecutionOutcome.FINISHED_SUCCESSFULLY == feedback.getOutcome()){
      return new SyncTaskStats(true, feedback.getResult().getNumberOfBytes());
    } else {
  ...
  public static SyncTaskStats fromMaps(
      Map<MetadataSyncTaskOperation, Metrics> metaSuccesses,
      Map<MetadataSyncTaskOperation, Integer> metaFailures,
      Metrics blockSuccesses, Integer blockFailures) {
    return new SyncTaskStats(metaSuccesses, metaFailures, blockSuccesses, blockFailures);
  }
  ...
  public String prettyPrint() {
    ...
    builder.append("--------");
    builder.append("\n");
    Set<Map.Entry<MetadataSyncTaskOperation, Integer>> failureEntries = metaFailures.entrySet();
  ...
  private void prettyPrintSuccesses(StringBuilder builder,
    ...
    builder.append(String.format("%-30s= %s" , "Files Created:", numberOfCreates));
    builder.append("\n");
    builder.append(String.format("%-30s= %s" , "Files Renamed:", numberOfRenames));
    builder.append("\n");
    builder.append(String.format("%-30s= %s" , "Files Deleted:", numberOfDeletes));
    builder.append("\n");
    builder.append(String.format("%-30s= %s" , "Bytes Copied:", formatBytes(numberOfBytes)));
    builder.append("\n");
  }
  ...
  private void prettyPrintFailures(StringBuilder builder,
    ...
    long numberOfFailures = entries
        ...
        .sum();
    builder.append(String.format("%-30s= %s" , "Total operations failed:", numberOfFailures));
    builder.append("\n");
  }
