...
public class TestSyncTaskStats {
  ...
  public void testFrom() {
    ...
    when(metadataFeedbackMock.getResult())
        .thenReturn(SyncTaskExecutionResult.emptyResult());
    when(metadataFeedbackMock.getOperation())
        .thenReturn(MetadataSyncTaskOperation.CREATE_DIRECTORY);
    ...
    assertThat(syncTaskStats.metaSuccesses)
        .containsOnlyKeys(MetadataSyncTaskOperation.CREATE_DIRECTORY);
  ...
  public void testFromWithBytes() {
    ...
    when(blockFeedbackMock.getResult()).thenReturn(new SyncTaskExecutionResult(
        ByteBuffer.wrap(new byte[0]), numberOfBytes));
    SyncTaskStats syncTaskStats = SyncTaskStats.from(blockFeedbackMock);
    assertThat(syncTaskStats.blockSuccesses).isEqualTo(Metrics.of(1,
        numberOfBytes));
  }
  ...
  public void testFromFailure() {
    ...
    when(metadataFeedbackMock.getOperation()).thenReturn(
    ...
    assertThat(syncTaskStats.metaFailures)
  ...
  public void testAppend() {
    ...
    failuresLeft.put(MetadataSyncTaskOperation.MULTIPART_COMPLETE,
        leftCompletes);
    ...
    failuresRight.put(MetadataSyncTaskOperation.MULTIPART_COMPLETE,
        rightCompletes);

    Metrics blockSuccessesRight = Metrics.of(rightCreateDirectories,
        rightSize);
    ...
    int actualCreateDirectories = leftCreateDirectories +
        rightCreateDirectories;
