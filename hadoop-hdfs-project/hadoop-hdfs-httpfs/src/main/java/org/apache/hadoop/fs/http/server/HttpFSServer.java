...
import static org.apache.hadoop.fs.http.client.HttpFSFileSystem.Operation.GETHOMEDIRECTORY;
import static org.apache.hadoop.fs.http.client.HttpFSFileSystem.Operation.INSTRUMENTATION;
...
public class HttpFSServer {
  ...
  public Response get(@PathParam("path") String path,
    ...
    try {
      ...
      LOG.error("params with illegal arguments", e);
    ...
    try {
      ...
      case OPEN: {
        ...
        if (noRedirect) {
          ...
          InputStream is = null;
          UserGroupInformation ugi = UserGroupInformation.createProxyUser(user.getShortUserName(),
      ...
      case GETHOMEDIRECTORY: {
        try {
          ...
          LOG.error("Error processing {}", GETHOMEDIRECTORY, e);
          response =
              Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).build();
          break;
        }
      ...
      case INSTRUMENTATION: {
        try {
          ...
          LOG.error("Error processing {}", INSTRUMENTATION, e);
          response =
              Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).build();
          break;
        }
        ...
        if (!userGroups.contains(HttpFSServerWebApp.get().getAdminGroup())) {
          LOG.error("User {} not in HttpFSServer admin group", user.getShortUserName());
      ...
      case LISTSTATUS_BATCH: {
        String startAfter = params.get(HttpFSParametersProvider.StartAfterParam.NAME,
      ...
      case GETSNAPSHOTDIFF: {
        ...
        FSOperations.FSGetSnapshotDiff command =
            new FSOperations.FSGetSnapshotDiff(path, oldSnapshotName, snapshotName);
      ...
      case GETSNAPSHOTDIFFLISTING: {
        ...
        String snapshotDiffStartPath =
            params.get(HttpFSParametersProvider.SnapshotDiffStartPathParam.NAME,
        ...
        FSOperations.FSGetSnapshotDiffListing command =
            new FSOperations.FSGetSnapshotDiffListing(path, oldSnapshotName, snapshotName,
      ...
      case CHECKACCESS: {
        ...
        FSOperations.FSAccess command =
            new FSOperations.FSAccess(path, FsAction.getFsAction(fsparam.value()));
    ...
    try {
      ...
      default: {
        throw new IOException(
            MessageFormat.format("Invalid HTTP DELETE operation [{0}]", op.value()));
      }
      }
    } catch (Exception e) {
      LOG.error("Error serving delete", e);
      Map<String, String> errorMsg = new HashMap<>();
      errorMsg.put("error", e.getMessage());
      response = Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity(errorMsg)
  ...
  public Response post(InputStream is,
    ...
    try {
      ...
      default: {
        throw new IOException(
            MessageFormat.format("Invalid HTTP POST operation [{0}]", op.value()));
      }
      }
    } catch (Exception e) {
      LOG.error("Error serving post", e);
      Map<String, String> errorMsg = new HashMap<>();
      errorMsg.put("error", e.getMessage());
      response = Response.status(Response.Status.INTERNAL_SERVER_ERROR)
          .entity(errorMsg)
  ...
  public Response put(InputStream is,
    ...
    try {
      ...
      case CREATE: {
        ...
        if (noRedirect) {
          ...
          Short unmaskedPermission =
              params.get(UnmaskedPermissionParam.NAME, UnmaskedPermissionParam.class);
          ...
          FSOperations.FSCreate command =
              new FSOperations.FSCreate(is, path, permission, override, replication, blockSize,
                  unmaskedPermission);
          fsExecute(user, command);
          AUDIT_LOG.info("[{}] permission [{}] override [{}] "
          ...
          response = Response.created(uriInfo.getAbsolutePath()).type(MediaType.APPLICATION_JSON)
              .entity(js).build();
        } else {
          response = Response.temporaryRedirect(redirectURL).build();
        }
      ...
      case SETXATTR: {
        ...
        FSOperations.FSSetXAttr command =
            new FSOperations.FSSetXAttr(path, xattrName, xattrValue, flag);
        fsExecute(user, command);
      ...
      case MKDIRS: {
        ...
        JSONObject json = fsExecute(user, command);
        AUDIT_LOG.info("[{}] permission [{}] unmaskedpermission [{}]", path, permission,
            unmaskedPermission);
        response = Response.ok(json).type(MediaType.APPLICATION_JSON).build();
        break;
      }
