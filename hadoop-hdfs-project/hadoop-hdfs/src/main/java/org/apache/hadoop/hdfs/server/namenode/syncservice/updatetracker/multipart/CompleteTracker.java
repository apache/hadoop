...
/**
 * Track finished/failed sync task in COMPLETE phase.
 */
public class CompleteTracker {

  private CurrentTasks<MultipartCompleteMetadataSyncTask> currentCompleteTasks;

  public CompleteTracker(MultipartCompleteMetadataSyncTask
      completeMetadataSyncTask, CurrentTasksFactory currentTasksFactory) {
    this.currentCompleteTasks =
        currentTasksFactory.create(completeMetadataSyncTask);
  }

  public void markFinished(UUID syncTaskId, Runnable trackerFinalizer) {
    Optional<MultipartCompleteMetadataSyncTask> syncTask =
        this.currentCompleteTasks.markFinished(syncTaskId);

    if (this.currentCompleteTasks.isFinished() && syncTask.isPresent()) {
  ...
  public boolean isTaskUnderTrack(UUID syncTaskId) {
    return currentCompleteTasks.isTaskUnderTrack(syncTaskId);
  }
}
