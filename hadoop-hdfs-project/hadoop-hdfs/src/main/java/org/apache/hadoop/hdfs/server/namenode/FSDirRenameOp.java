...
class FSDirRenameOp {
  ...
  private static class RenameOperation {
    ...
    private final Optional<QuotaCounts> srcSubTreeCountOp;
    private final Optional<QuotaCounts> dstSubTreeCountOp;
    ...
    RenameOperation(FSDirectory fsd, INodesInPath srcIIP, INodesInPath dstIIP,
      ...
      this.sameStoragePolicy = isSameStoragePolicy(srcIIP, dstIIP);
    ...
    boolean isSameStoragePolicy(INodesInPath srcIIP, INodesInPath dstIIP) {
      final INode src = srcIIP.getLastINode();
      final INode dst = dstIIP.getLastINode();
      // If the source INode has a storagePolicyID, we should also use
      // its storagePolicyId for calculation after the rename operation.
      if (srcIIP.getLastINode().isSetStoragePolicy()) {
        return true;
      }
    ...
    long removeSrc() throws IOException {
      ...
      if (removedNum == -1) {
        ...
        Optional<QuotaCounts> countOp = sameStoragePolicy ?
            srcSubTreeCountOp : Optional.empty();
    ...
    long removeDst() {
      ...
      if (removedNum != -1) {
        ...
        // update the quota count if necessary
        fsd.updateCountForDelete(oldDstChild, dstIIP, dstSubTreeCountOp);
    ...
    INodesInPath addSourceToDestination() {
      ...
      return fsd.addLastINodeNoQuotaCheck(dstParentIIP, toDst, srcSubTreeCountOp);
    }
    ...
    void restoreDst(BlockStoragePolicySuite bsps) {
      ...
      if (dstParent.isWithSnapshot()) {
        ...
        fsd.addLastINodeNoQuotaCheck(dstParentIIP, oldDstChild, dstSubTreeCountOp);
      }
