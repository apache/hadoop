...
public class DatanodeManager {
   ...
   *
   * <p>The group topology is therefore always created via
   * {@link NetworkTopology#getInstance}, which honors {@code net.topology.impl}:
   * a plain {@link NetworkTopology} by default, or a
   * {@code NetworkTopologyWithNodeGroup} for a node-group deployment (so a
   * topology-typed replicator's {@code initialize()} check still passes). It
   * never creates a {@link DFSNetworkTopology}. These impls track no storage
   * counts; the default placement policy selects a random eligible leaf and
   * validates the storage type against the LIVE descriptor's storages, which is
   * always current.
   */
