...
public class RunningDatanodeState implements DatanodeState {
  ...
  private Callable<EndpointStateMachine.EndPointStates>
    ...
    case REGISTER:
      ...
      return task;
    case HEARTBEAT:
      HeartbeatEndpointTask hbTask = new HeartbeatEndpointTask(endpoint, conf);
      hbTask.setContainerNodeIDProto(getContainerNodeID());
      return hbTask;
    case SHUTDOWN:
      break;
    default:
   ...
   * Machine will be declared to be in Running state. otherwise we will remain
   * in Handshake State. If any endpoint tells us to shutdown, we will move the
   * whole container State machine to Shutdown state and exit from the state
   * machine running loop.
   *
