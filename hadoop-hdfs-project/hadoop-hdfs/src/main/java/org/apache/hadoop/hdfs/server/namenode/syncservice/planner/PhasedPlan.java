...
/**
 * Phased plan for syncing data to remote storage.
 */
public class PhasedPlan {
   ...
   * Consider this. Need to find something for the decision whether we are in
   * the create files phase and need to create a multipart plan when we do that.
   */
  ...
  public boolean hasNoDownstreamTasksLeft(Phases currentPhase) {
    switch (currentPhase) {
    case RENAMES_TO_TEMP:
      return this.renameToTemporaryName.isEmpty() &&
          hasNoDownstreamTasksLeft(DELETES);
    case DELETES:
      return this.deleteMetadataSyncTasks.isEmpty() &&
          hasNoDownstreamTasksLeft(RENAMES_TO_FINAL);
    case RENAMES_TO_FINAL:
      return this.renameToFinalName.isEmpty() &&
          hasNoDownstreamTasksLeft(CREATE_DIRS);
    case CREATE_DIRS:
      return this.createDirSyncTasks.isEmpty() &&
          hasNoDownstreamTasksLeft(CREATE_FILES);
    case CREATE_FILES:
      return this.createFileSyncTasks.isEmpty() &&
          hasNoDownstreamTasksLeft(FINISHED);
    case FINISHED:
      return true;
    default:
  ...
  public void filter(Path pathInAliasMap) {
    this.createFileSyncTasks =
        this.createFileSyncTasks
            .stream()
            .filter(syncTask -> syncTask.getUri().getPath().equals(
                pathInAliasMap.toString()))
