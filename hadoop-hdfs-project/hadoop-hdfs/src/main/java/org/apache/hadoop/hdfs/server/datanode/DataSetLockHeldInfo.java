...
public class DataSetLockHeldInfo {
  ...
  private long getLockHeldInterval() {
    if (lockInfo.size() <= 0) {
      return 0;
    }
    return lockInfo.get(lockInfo.size() - 1).getLockHeldInterval();
  }
  ...
  private void validateLockInfo() {
    if (lockInfo.size() <= 0) {
      LOG.error("Found inValidate lock held info, " + StringUtils.getStackTrace(Thread.currentThread()));
    }
  }
