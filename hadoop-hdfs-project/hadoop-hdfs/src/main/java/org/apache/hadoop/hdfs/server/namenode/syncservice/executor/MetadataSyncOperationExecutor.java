...
/**
 * Metadata sync executor created on NameNode for provided storage
 * sync service.
 */
public final class MetadataSyncOperationExecutor {
  ...
  public SyncTaskExecutionResult execute(MetadataSyncTask metadataSyncTask)
    ...
    case MULTIPART_COMPLETE:
      ...
      return doMultiPartComplete(mpcSyncTask.getUri(),
          mpcSyncTask.getUploadHandle(), mpcSyncTask.getPartHandles());
    case TOUCH_FILE:
    ...
    case RENAME_FILE:
      ...
      return doRename(rfSyncTask.getUri(), rfSyncTask.getRenamedTo());
    case RENAME_DIRECTORY:
      ...
      return doRenameDirectory(rdSyncTask.getUri(), rdSyncTask.getRenamedTo());
    default:
      LOG.warn("MetadataSyncTask {} can not be executed: operation {} not " +
              "implemented in {}",
          metadataSyncTask.getSyncTaskId(), metadataSyncTask.getOperation(),
  ...
  private SyncTaskExecutionResult doMultiPartInit(URI uri) throws IOException {
    Path filePath = new Path(uri);
    FileSystem fs = FileSystem.get(uri, conf);
    MultipartUploader mpu = fs.createMultipartUploader(filePath).build();
    UploadHandle uploadHandle;
    try {
      uploadHandle = mpu.startUpload(filePath).get();
  ...
  private SyncTaskExecutionResult doMultiPartComplete(URI uri,
    ...
    FileSystem fs = FileSystem.get(uri, conf);
    MultipartUploader mpu = fs.createMultipartUploader(filePath).build();
    Map<Integer, PartHandle> partList = new HashMap<>();
    for (ByteBuffer ph : partHandles) {
      PartHandle partHandle = BBPartHandle.from(ph);
      Integer partNumber = mpu.getPartNumber(partHandle);
      partList.put(partNumber, partHandle);
    }
    PathHandle etag = awaitFuture(mpu.complete(
        BBUploadHandle.from(uploadHandle), filePath, partList));
