...
/**
 * Sync service worker for provided storage.
 */
public class SyncServiceSatisfierDatanodeWorker {
  ...
  private SyncTaskExecutionFeedbackCollector executionFeedbackCollector;

  public SyncServiceSatisfierDatanodeWorker(Configuration conf,
    ...
    this.syncOperationExecutor =
        BlockSyncOperationExecutor.createOnDataNode(conf,
          (locatedBlock, config) -> {
              try {
                return BlockSyncReaderFactory.createBlockReader(
                    dataNode, locatedBlock, config);
              } catch (IOException e) {
                throw new RuntimeException(e);
              }
            }
          );
    this.executionFeedbackCollector =
        new SyncTaskExecutionFeedbackCollector();
  }
  ...
  public void waitToFinishWorkerThread() {
    try {
      ...
      LOG.warn("SyncServiceSatisfierDatanodeWorker interrupted during " +
    ...
    for (BlockSyncTask blockSyncTask : blockSyncTasks) {
      try {
        executorService.submit(new BlockSyncTaskRunner(blockSyncTask,
            ...
            syncTaskExecutionFeedback -> executionFeedbackCollector
