...
class BPServiceActor implements Runnable {
  ...
  private Object fullBlockReportLeaseIdLock = new Object();
  ...
  BPServiceActor(String serviceId, String nnId, InetSocketAddress nnAddr,
    ...
    synchronized (fullBlockReportLeaseIdLock) {
    ...
    brExecutorService = Executors.newSingleThreadExecutor(
        new ThreadFactoryBuilder().setDaemon(true)
            .setNameFormat("br-executor-%d").build());
  }
    ...
    while (shouldRun()) {
      try {
        ...
        if (sendHeartbeat) {
          ...
          // -- Bytes remaining
          //
          boolean requestBlockReportLease = false;
          synchronized (fullBlockReportLeaseIdLock) {
            requestBlockReportLease = (fullBlockReportLeaseId == 0) &&
        ...
        try {
          ...
          synchronized (fullBlockReportLeaseIdLock) {
            if ((fullBlockReportLeaseId != 0) || forceFullBr) {
              mirror = fullBlockReportLeaseId;
              fullBlockReportLeaseId = 0;
            }
          }
          if ((mirror != 0) || forceFullBr) {
            cmds = blockReport(mirror);
          }
