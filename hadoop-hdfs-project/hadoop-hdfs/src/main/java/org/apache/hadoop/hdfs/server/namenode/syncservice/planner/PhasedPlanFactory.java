...
/**
 * Factory of sync service phased plan.
 */
public class PhasedPlanFactory {
  ...
  public PhasedPlanFactory(FilePlanner filePlanner,
      DirectoryPlanner directoryPlanner) {
  ...
  public PhasedPlan createFromPartitionedDiffReport(PartitionedDiffReport
      partitionedDiffReport, ProvidedVolumeInfo syncMount,
      String sourceSnapshot, Optional<Integer> sourceSnapshotId,
    ...
    return new PhasedPlan(renameToTemporaryName,
        deleteSyncTasks.getAllTasksForDelete(),
  ...
  private List<SyncTask> createRenameToTemporaryNameSyncTasks(
      List<PartitionedDiffReport.RenameEntryWithTempName> renames,
      ProvidedVolumeInfo syncMount, int targetSnapshotId) {
  ...
  private Optional<SyncTask> convertRenameToTempNameToSyncTask(
      PartitionedDiffReport.RenameEntryWithTempName entryWithTempName,
      ProvidedVolumeInfo syncMount, int targetSnapshotId) {
      ...
      try {
        ...
        return Optional.of(
            SyncTask.renameFile(sourceRemoteURI,
                renameToRemoteURI, blocks, syncMount.getId()));
      } catch (IOException e) {
        LOG.error("Could not createRenameSyncTask for {}",
            diffEntry.getTargetPath());
        return Optional.empty();
      }
    } else if (diffEntry.getInodeType() ==
        SnapshotDiffReport.INodeType.SYMLINK) {
      LOG.warn("Symlinks are not supported. Skipping rename task.");
      return Optional.empty();
    } else {
      throw new RuntimeException("Unknown INodeType: " +
          "can't create rename task");
    }
  }

  private FileAndDirsSyncTasks createDeleteSyncTasks(
      List<TranslatedEntry> deletes, ProvidedVolumeInfo syncMount,
      String sourceSnapshot, int sourceSnapshotId) {
  ...
  private FileAndDirsSyncTasks createDeleteSyncTaskTree(
      TranslatedEntry deleteEntry, ProvidedVolumeInfo syncMount,
      String sourceSnapshot, int sourceSnapshotId) {
    ...
    if (diffEntry.getInodeType() == SnapshotDiffReport.INodeType.DIRECTORY) {
      ...
      URI sourceRemoteURI =
          createRemotePath(syncMount, deleteEntry.getTranslatedName());
      FileAndDirsSyncTasks plan = new FileAndDirsSyncTasks();
      try {
        ...
        plan.addFileSync(SyncTask.deleteFile(
            sourceRemoteURI, blocks, syncMount.getId()));
      } catch (IOException e) {
        LOG.error("Could not create DeleteSyncTask for {}",
            diffEntry.getTargetPath());
      }
      return plan;
    } else {
      LOG.warn("Unsupported INode type: {}", diffEntry.getInodeType());
      return new FileAndDirsSyncTasks();
    }
  }

  private FileAndDirsSyncTasks createCreateSyncTasks(
      List<TranslatedEntry> creates,
      ProvidedVolumeInfo syncMount, int targetSnapshotId) {
    FileAndDirsSyncTasks plan = new FileAndDirsSyncTasks();
    for (TranslatedEntry create : creates) {
      FileAndDirsSyncTasks subPlan =
          createCreateSyncTaskTree(create, syncMount, targetSnapshotId);
      plan.append(subPlan);
    }
    return plan;
  }
  ...
  FileAndDirsSyncTasks createCreateSyncTaskTree(TranslatedEntry createEntry,
    ...
    if (diffEntry.getInodeType() == SnapshotDiffReport.INodeType.DIRECTORY) {
      FileAndDirsSyncTasks plan = directoryPlanner.createPlanForDirectory(
          diffEntry, createEntry.getTranslatedName(),
          syncMount, targetSnapshotId);
      return plan;
    } else if (diffEntry.getInodeType() == SnapshotDiffReport.INodeType.FILE) {
      try {
        FileAndDirsSyncTasks plan = new FileAndDirsSyncTasks();
        SyncTask createFile = filePlanner.createPlanTreeNodeForCreatedFile(
            syncMount, targetSnapshotId, diffEntry,
            createEntry.getTranslatedName());
  ...
  private FileAndDirsSyncTasks createCreatesFromRenamesSyncTasks(
      List<DiffReportEntry> createsFromRenames, ProvidedVolumeInfo syncMount,
      int targetSnapshotId) {
    ...
    for (DiffReportEntry createEntry : createEntries) {
      try {
        ...
        } else if (createEntry.getInodeType() ==
            SnapshotDiffReport.INodeType.FILE) {
          String targetName =
              DFSUtil.bytes2String(createEntry.getSourcePath());
          SyncTask metadataSyncTask =
              filePlanner.createPlanTreeNodeForCreatedFile(syncMount,
              targetSnapshotId, createEntry, targetName);
          plan.addFileSync(metadataSyncTask);
        } else {
          LOG.warn("Unsupported INode type: {}", createEntry.getInodeType());
          return null;
        }
      } catch (IOException e) {
  ...
  private List<SyncTask> createModifiedSyncTasks(
      List<TranslatedEntry> modifies, ProvidedVolumeInfo syncMount,
      int targetSnapshotId) {
      ...
      try {
        SyncTask modifyFile =
            filePlanner.createModifiedFileSyncTasks(targetSnapshotId,
                diffEntry.getSourcePath(), modifiedEntry.getTranslatedName(),
                syncMount);
        return Optional.of(modifyFile);
      } catch (IOException e) {
        //TODO Handle errors
        throw new RuntimeException("Error creating ModifiedSyncTask for " +
