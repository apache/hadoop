...
public class TestFilePlanner {
  ...
  public void createFileWithZeroSize() throws Exception {
    ...
    ProvidedVolumeInfo syncMount =
        new ProvidedVolumeInfo(syncMountId, syncMountLocalPath,
            remoteLocation.toString(), MountMode.WRITEBACK);

    INodeFile iNodeFile = new INodeFile(44L, iNodeName.getBytes(), PERM, 0L, 0L,
        null, (short) 0, 0L);

    SyncTask createdFileSyncTasks = filePlanner.createCreatedFileSyncTasks(
        targetSnapshotId, iNodeFile, syncMount, sourcePath);

    assertThat(createdFileSyncTasks).isNotNull();
    assertThat(createdFileSyncTasks.getOperation()).isEqualTo(
        SyncTaskOperation.TOUCH_FILE);
    assertThat(createdFileSyncTasks.getSyncMountId()).isEqualTo(
        syncMountId.toString());
    assertThat(createdFileSyncTasks.getUri()).isEqualTo(
