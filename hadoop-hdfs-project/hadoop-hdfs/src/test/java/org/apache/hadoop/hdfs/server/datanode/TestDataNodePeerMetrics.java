...
public class TestDataNodePeerMetrics {
  ...
  public void testRemoveStaleRecord() throws Exception {
    ...
    final DataNodePeerMetrics peerMetrics =
        DataNodePeerMetrics.create("Sample-DataNode");
    final MutableRollingAverages rollingAverages =
        peerMetrics.getSendPacketDownstreamRollingAverages();
    ...
    GenericTestUtils.waitFor(
        new Supplier<Boolean>() {
          @Override
          public Boolean get() {
            return rollingAverages.getStats(numSamples).size() > 0;
          }
        }, 500, 5000);
    ...
    GenericTestUtils.waitFor(
        new Supplier<Boolean>() {
          @Override
          public Boolean get() {
            return rollingAverages.getStats(numSamples).isEmpty();
          }
        }, 500, 10000);
