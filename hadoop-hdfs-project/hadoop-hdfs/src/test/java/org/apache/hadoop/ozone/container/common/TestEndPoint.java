...
public class TestEndPoint {
  ...
  public void testHeartbeatTaskToInvalidNode() throws Exception {
    ...
    try(EndpointStateMachine rpcEndpoint =
      ...
      // if a  server endpoint fails .. we don't move away from the HB state.
      // we just try to keep HB-ing that SCM unless some command is send by
      // the active SCMs to cleanup this endpoint.
      Assert.assertEquals(EndpointStateMachine.EndPointStates.HEARTBEAT,
          rpcEndpoint.getState());
    }
  }
