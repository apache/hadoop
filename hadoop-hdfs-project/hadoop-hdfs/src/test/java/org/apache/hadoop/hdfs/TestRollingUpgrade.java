...
public class TestRollingUpgrade {
  ...
  public void testDFSAdminRollingUpgradeCommands() throws Exception {
    ...
    try (MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf, baseDir).numDataNodes(0).build()) {
      cluster.waitActive();
  ...
  public void testRollingUpgradeWithQJM() throws Exception {
    ...
    final Configuration conf = new HdfsConfiguration();
    try (MiniJournalCluster mjc = new MiniJournalCluster.Builder(conf, baseDir)
        .build()) {
      mjc.waitActive();
...

      {
        ...
        final MiniDFSCluster cluster = new MiniDFSCluster.Builder(conf, baseDir)
            .numDataNodes(0)
  ...
  public void testRollback() throws Exception {
    // start a cluster
    final Configuration conf = new HdfsConfiguration();
    try (MiniDFSCluster cluster  = new MiniDFSCluster.Builder(conf, baseDir).numDataNodes(1).build()) {
      cluster.waitActive();
  ...
  private void testFinalize(int nnCount, boolean skipImageDeltaCheck)
    ...
    try {
      cluster = new MiniQJMHACluster.Builder(conf, baseDir).setNumNameNodes(nnCount).build();
  ...
  private void testQuery(int nnCount) throws Exception{
    final Configuration conf = new HdfsConfiguration();
    try (MiniQJMHACluster cluster = new MiniQJMHACluster.Builder(conf, baseDir).setNumNameNodes(nnCount).build()) {
      MiniDFSCluster dfsCluster = cluster.getDfsCluster();
      dfsCluster.waitActive();
  ...
  public void testRollBackImage() throws Exception {
    ...
    try {
      cluster = new MiniQJMHACluster.Builder(conf, baseDir).setNumNameNodes(2).build();
      MiniDFSCluster dfsCluster = cluster.getDfsCluster();
      dfsCluster.waitActive();
      dfsCluster.transitionToActive(0);
      DistributedFileSystem dfs = dfsCluster.getFileSystem(0);
  ...
  public void testCheckpoint(int nnCount) throws IOException, InterruptedException {
    ...
    try (MiniQJMHACluster cluster = new MiniQJMHACluster.Builder(conf, baseDir).setNumNameNodes(nnCount)
        .build()) {
      MiniDFSCluster dfsCluster = cluster.getDfsCluster();
      dfsCluster.waitActive();
  ...
  public void testCheckpointWithSNN() throws Exception {
    ...
    try {
      ...
      cluster = new MiniDFSCluster.Builder(conf, baseDir).build();
      cluster.waitActive();
