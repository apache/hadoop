...
public class TestDataSetLockManager {
  ...
  public void testBaseFunc() {
    ...
    AutoCloseDataSetLock lock = manager.writeLock("BPtest", LockLevel.BLOCK_POOl, "BPtest");
    AutoCloseDataSetLock lock1 = manager.readLock("BPtest", LockLevel.BLOCK_POOl, "BPtest");
    lock1.close();
    lock.close();
    ...
    AutoCloseDataSetLock lock2 = manager.writeLock("BPtest", LockLevel.VOLUME, "BPtest", "Volumetest");
    AutoCloseDataSetLock lock3 = manager.readLock("BPtest", LockLevel.VOLUME, "BPtest", "Volumetest");
    lock3.close();
    lock2.close();
    ...
    AutoCloseDataSetLock lock4 = manager.writeLock("BPtest", LockLevel.BLOCK_POOl, "BPtest");
    AutoCloseDataSetLock lock5 = manager.readLock("BPtest", LockLevel.VOLUME, "BPtest", "Volumetest");
    lock5.close();
    lock4.close();
  ...
  public void testLockHeldInfo() {
    ...
    try (AutoCloseDataSetLock lock = manager.writeLock("acquireLockTest", LockLevel.BLOCK_POOl, "test")) {
    ...
    try (AutoCloseDataSetLock bpLock = manager.readLock("acquireLockTest", LockLevel.BLOCK_POOl, "test")) {
      DataSetLockHeldInfo lockHeldInfo = bpLock.getLockHeldInfo();
      assertEquals(1, lockHeldInfo.getLockInfoSize());
      lock1.add(bpLock);

      try (AutoCloseDataSetLock volumeLock = manager.writeLock("acquireLockTest", LockLevel.VOLUME, "test", "v1")) {
