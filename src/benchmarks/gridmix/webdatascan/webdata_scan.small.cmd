...
set INDIR="%VARCOMPSEQ%/{part-00000,part-00001,part-00002}"
FOR /F "delims=" %%a in ('%CYGWIN_HOME%\bin\date +%%F-%%H-%%M-%%S-%%N') DO (
 SET DATE=%%a
)


set OUTDIR=perf-out/webdata-scan-out-dir-small_%Date%
CALL %HADOOP_HOME%\bin\hadoop dfs -rmr %OUTDIR%

CALL %HADOOP_HOME%\bin\hadoop jar %APP_JAR% loadgen -keepmap 1 -keepred 5 ^
