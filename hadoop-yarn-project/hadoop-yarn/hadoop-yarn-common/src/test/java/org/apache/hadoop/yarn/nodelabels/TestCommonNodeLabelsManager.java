...
public class TestCommonNodeLabelsManager extends NodeLabelTestBase {
  ...
  public void testAddRemovelabel() throws Exception {
    ...
    verifyNodeLabelAdded(new HashSet<>(Collections.singletonList("hello")), mgr.lastAddedlabels);
    ...
    // Remove some label
    mgr.removeFromClusterNodeLabels(Collections.singletonList("hello"));
    assertCollectionEquals(new HashSet<>(Collections.singletonList("hello")), mgr.lastRemovedlabels);
    Assert.assertTrue(mgr.getClusterNodeLabelNames().containsAll(
