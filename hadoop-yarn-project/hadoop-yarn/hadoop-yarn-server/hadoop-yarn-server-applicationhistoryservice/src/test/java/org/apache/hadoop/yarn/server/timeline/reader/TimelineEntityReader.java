...
public class TimelineEntityReader implements MessageBodyReader<TimelineEntity> {
  ...
  public TimelineEntity readFrom(Class<TimelineEntity> type, Type genericType,
    ...
    try {
      TimelineEntity timelineEntity = objectMapper.readValue(entityStream, TimelineEntity.class);
      if (timelineEntity.getPrimaryFilters() == null) {
        timelineEntity.setPrimaryFilters(new HashMap<>());
      }
      if (timelineEntity.getOtherInfo() == null) {
        timelineEntity.setOtherInfo(new HashMap<>());
      }
