...
public class TestAHSWebServices extends JerseyTest {
  ...
  public void testContainerLogsForFinishedApps() throws Exception {
    ...
    response = target
        ...
        .path("applicationhistory")
        .path("containerlogs")
        .path(containerId1ForApp100.toString())
        .path(fileName)
        .queryParam("user.name", user)
        .request(MediaType.TEXT_PLAIN)
        .get(Response.class);
  ...
  public void testContainerLogsMetaForRunningApps() throws Exception {
    ...
    response = target
        ...
        .path("applicationhistory")
        .path("containers")
        .path(containerId1000.toString())
        .path("logs")
        .queryParam(YarnWebServiceParams.NM_ID, "invalid-nm:1234")
        .queryParam("user.name", user)
        .request(MediaType.APPLICATION_JSON)
        .get(Response.class);
    responseText = response.readEntity(new GenericType<List<ContainerLogsInfo>>() {
    });
    assertTrue(responseText.size() == 2);
    for (ContainerLogsInfo logInfo : responseText) {
      if(logInfo.getLogType().equals(
