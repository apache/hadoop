...
public class NumaResourceHandlerImpl implements ResourceHandler {
  ...
  public List<PrivilegedOperation> preStart(Container container)
    ...
    if (numaAllocation != null) {
      ...
      for(String memNode: numaAllocation.getMemNodes()){
        memNodes.append(memNode).append(",");
      }
      args.add(
          "--interleave=" + memNodes);
      StringBuilder cpuNodes = new StringBuilder();
      for(String cpuNode: numaAllocation.getCpuNodes()){
        cpuNodes.append(cpuNode).append(",");
      }
      args.add(
          "--cpunodebind=" + cpuNodes);
