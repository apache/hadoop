...
public class TestDockerCommandExecutor {
  ...
  public void testGetContainerStatus() throws Exception {
    for (DockerContainerStatus status : DockerContainerStatus.values()) {
      when(mockExecutor.executePrivilegedOperation(anyListOf(String.class),
          any(PrivilegedOperation.class), any(File.class),
          Matchers.<Map<String, String>>any(), eq(true), eq(false)))
