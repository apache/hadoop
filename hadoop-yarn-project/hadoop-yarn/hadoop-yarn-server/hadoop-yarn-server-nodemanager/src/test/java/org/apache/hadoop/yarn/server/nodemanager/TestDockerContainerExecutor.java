...
public class TestDockerContainerExecutor {
  ...
  @Test
  public void testLaunchContainerJava() throws IOException {
    if (!shouldRun()) {
      LOG.warn("Docker not installed, aborting test.");
      return;
    }
    ...
    Map<String, String> env = new HashMap<String, String>();
    env.put(YarnConfiguration.NM_DOCKER_CONTAINER_EXECUTOR_IMAGE_NAME, testImage);
    env.put(ApplicationConstants.Environment.JAVA_HOME.name(), "/usr/lib/jvm/java-8-openjdk-amd64");
    ContainerId cId = getNextContainerId();
    int ret = runAndBlock(
      cId, env, "$JAVA_HOME/bin/java", "-version");
