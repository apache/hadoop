...
//    60, 28
  ...
  void testSimpleMinMaxResourceConfigurartionPerQueue()
    ...
    Assertions.assertEquals(QUEUE_A_MAXRES,
    ...
    Assertions.assertEquals(QUEUE_B_MINRES,
    ...
    Assertions.assertEquals(QUEUE_B_MAXRES,
    ...
    Assertions.assertEquals(QUEUE_C_MINRES,
    ...
    Assertions.assertEquals(QUEUE_C_MAXRES,
  ...
  @Test
  void testNormalizationAfterNodeRemoval() throws Exception {
  ...
  @Test
  void testEffectiveMinMaxResourceConfigurartionPerQueue()
      throws Exception {
    ...
    Assertions.assertEquals(QUEUE_A_MINRES,
        qA.usageTracker.getQueueResourceQuotas().getConfiguredMinResource(), "Min resource configured for QUEUEA is not correct");
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA.usageTracker.getQueueResourceQuotas().getConfiguredMaxResource(), "Max resource configured for QUEUEA is not correct");
    Assertions.assertEquals(QUEUE_A_MINRES,
        qA.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEA is not correct");
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA is not correct");

    LeafQueue qB = (LeafQueue) cs.getQueue(QUEUEB);
    Assertions.assertNotNull(qB);
    Assertions.assertEquals(QUEUE_B_MINRES,
        qB.usageTracker.getQueueResourceQuotas().getConfiguredMinResource(), "Min resource configured for QUEUEB is not correct");
    Assertions.assertEquals(QUEUE_B_MAXRES,
        qB.usageTracker.getQueueResourceQuotas().getConfiguredMaxResource(), "Max resource configured for QUEUEB is not correct");
    Assertions.assertEquals(QUEUE_B_MINRES,
        qB.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEB is not correct");
    Assertions.assertEquals(QUEUE_B_MAXRES,
        qB.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEB is not correct");

    LeafQueue qC = (LeafQueue) cs.getQueue(QUEUEC);
    Assertions.assertNotNull(qC);
    Assertions.assertEquals(QUEUE_C_MINRES,
        qC.usageTracker.getQueueResourceQuotas().getConfiguredMinResource(), "Min resource configured for QUEUEC is not correct");
    Assertions.assertEquals(QUEUE_C_MAXRES,
        qC.usageTracker.getQueueResourceQuotas().getConfiguredMaxResource(), "Max resource configured for QUEUEC is not correct");
    Assertions.assertEquals(QUEUE_C_MINRES,
        qC.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEC is not correct");
    Assertions.assertEquals(QUEUE_C_MAXRES,
        qC.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEC is not correct");

    rm.stop();
  }

  @Test
  void testSimpleValidateAbsoluteResourceConfig() throws Exception {
    /**
    ...
    Assertions.assertEquals(QUEUE_A1_MINRES,
        qA1.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEA1 is not correct");
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA1.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA1 is not correct");

    LeafQueue qA2 = (LeafQueue) cs.getQueue(QUEUEA2);
    Assertions.assertEquals(QUEUE_A2_MINRES,
        qA2.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEA2 is not correct");
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA2.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA2 is not correct");

    LeafQueue qB1 = (LeafQueue) cs.getQueue(QUEUEB1);
    Assertions.assertNotNull(qB1);
    Assertions.assertEquals(QUEUE_B1_MINRES,
        qB1.usageTracker.getQueueResourceQuotas().getConfiguredMinResource(), "Min resource configured for QUEUEB1 is not correct");
    Assertions.assertEquals(QUEUE_B_MAXRES,
        qB1.usageTracker.getQueueResourceQuotas().getConfiguredMaxResource(), "Max resource configured for QUEUEB1 is not correct");
    Assertions.assertEquals(QUEUE_B1_MINRES,
        qB1.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEB1 is not correct");
    Assertions.assertEquals(QUEUE_B_MAXRES,
        qB1.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEB1 is not correct");
  ...
  @Test
  void testComplexValidateAbsoluteResourceConfig() throws Exception {
    /**
     * Queue structure is as follows.
     *   root
     *  / | \
  ...
  @Test
  void testValidateAbsoluteResourceConfig() throws Exception {
    /**
  ...
  @Test
  void testDownscalingForLabels() throws Exception {
    CapacitySchedulerConfiguration csConf = setupSimpleQueueConfiguration(false);
    ...
    Assertions.assertTrue(Resources.greaterThan(cs.getResourceCalculator(), cs.getClusterResource(),
            root.getEffectiveCapacity(X_LABEL), childrenResource),
        "Children of root have more resource than overall cluster resource");
  }

  @Test
  void testEffectiveResourceAfterReducingClusterResource()
      throws Exception {
    // create conf with basic queue configuration.
    CapacitySchedulerConfiguration csConf = setupSimpleQueueConfiguration(
        false);
    ...
    // 128GB and 20 cores.
    Assertions.assertEquals(QUEUEA_REDUCED,
        qA.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEA is not correct");
    Assertions.assertEquals(QUEUEMAX_REDUCED,
        qA.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA is not correct");

    Assertions.assertEquals(QUEUEB_REDUCED,
        qB.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEB is not correct");
    Assertions.assertEquals(QUEUEMAX_REDUCED,
        qB.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEB is not correct");

    Assertions.assertEquals(QUEUEC_REDUCED,
        qC.usageTracker.getQueueResourceQuotas().getEffectiveMinResource(), "Effective Min resource for QUEUEC is not correct");
    Assertions.assertEquals(QUEUEMAX_REDUCED,
        qC.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEC is not correct");
  ...
  @Test
  void testEffectiveResourceAfterIncreasingClusterResource()
      throws Exception {
    // create conf with basic queue configuration.
    CapacitySchedulerConfiguration csConf = setupComplexQueueConfiguration(
        false);
    ...
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA is not correct");
    Assertions.assertEquals(0.4,
        qA.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEA is not correct");
    Assertions.assertEquals(0.8,
        qA.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEA is not correct");
    ...
    Assertions.assertEquals(0.2,
        qB.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEB is not correct");
    Assertions.assertEquals(0.6,
        qB.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEB is not correct");
    ...
    Assertions.assertEquals(0.1,
        qC.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEC is not correct");
    Assertions.assertEquals(0.6,
        qC.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEC is not correct");
    ...
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA1.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA1 is not correct");
    Assertions.assertEquals(0.2,
        qA1.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEA1 is not correct");
    Assertions.assertEquals(0.8,
        qA1.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEA1 is not correct");
    ...
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA2.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA2 is not correct");
    Assertions.assertEquals(0.2,
        qA2.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEA2 is not correct");
    Assertions.assertEquals(0.8,
        qA2.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEA2 is not correct");
    ...
    Assertions.assertEquals(QUEUE_B_MAXRES,
        qB1.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEB1 is not correct");
    Assertions.assertEquals(0.16,
        qB1.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEB1 is not correct");
    Assertions.assertEquals(0.6,
        qB1.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEB1 is not correct");
    ...
    Assertions.assertEquals(QUEUE_A_MAXRES,
        qA.usageTracker.getQueueResourceQuotas().getEffectiveMaxResource(), "Effective Max resource for QUEUEA is not correct");
    Assertions.assertEquals(0.266,
        qA.getAbsoluteCapacity(), DELTA, "Absolute capacity for QUEUEA is not correct");
    Assertions.assertEquals(0.533,
        qA.getAbsoluteMaximumCapacity(), DELTA, "Absolute Max capacity for QUEUEA is not correct");
