...
public class TestContainerAllocation {
  ...
  public void testContainerRejectionWhenAskBeyondDynamicMax()
    ...
    // Ask 3GB should also fail because node max allocation is 2GB.
    try {
      am1.allocate("*", 3 * GB, 1, null);
      Assert.fail("expect a failure because request exceeds"
          + " max node allocation");
    } catch (Exception e) {
    ...
    CapacityScheduler cs = (CapacityScheduler)rm1.getResourceScheduler();
    cs.setForceConfiguredMaxAllocation(false);
    rm1.registerNode("h2:1234", 3 * GB, 1);

    // Now ask 4 GB still fails
    submitFailed = false;
    try {
