...
public class TestQueueMetrics {
  ...
  @Test public void testDefaultSingleQueueMetrics() {
    ...
    metrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 100, 0));
    metrics.incrPendingResources(user, 5, Resources.createResource(3*GB, 3, 3, 7));
    ...
    metrics.allocateResources(user, 3, Resources.createResource(2*GB, 2, 2, 3), true);
    checkResources(queueSource, 6*GB, 6, 6, 3, 3, 0, 100*GB, 100, 100, 9*GB, 9, 9, 2, 0, 0, 0, 0);

    metrics.releaseResources(user, 1, Resources.createResource(2*GB, 2, 2, 3));
  ...
  @Test public void testSingleQueueWithUserMetrics() {
    ...
    metrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 100, 0));
    metrics.setAvailableResourcesToUser(user, Resources.createResource(10*GB, 10, 10, (1 << 10) - 1));
    metrics.incrPendingResources(user, 5, Resources.createResource(3*GB, 3, 3, 7));
    // Available resources is set externally, as it depends on dynamic
    // configurable cluster/queue resources
    checkResources(queueSource, 0, 0, 0, 0, 0, 0, 100*GB, 100, 100, 15*GB, 15, 15, 5, 0, 0, 0, 0);
  ...
  @Test public void testTwoLevelWithUserMetrics() {
    ...
    parentMetrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 100, 0));
    metrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 100, 0));
    parentMetrics.setAvailableResourcesToUser(user, Resources.createResource(10*GB, 10, 10, (1 << 10) - 1));
    metrics.setAvailableResourcesToUser(user, Resources.createResource(10*GB, 10, 10, (1 << 10) - 1));
    metrics.incrPendingResources(user, 5, Resources.createResource(3*GB, 3, 3, 7));
    checkResources(queueSource, 0, 0, 0, 0, 0, 0, 100*GB, 100, 100, 15*GB, 15, 15, 5, 0, 0, 0, 0);
    ...
    metrics.allocateResources(user, 3, Resources.createResource(2*GB, 2, 2, 3), true);
    metrics.reserveResource(user, Resources.createResource(3*GB, 3, 3, 7));
    // Available resources is set externally, as it depends on dynamic
    // configurable cluster/queue resources
    ...
    metrics.releaseResources(user, 1, Resources.createResource(2*GB, 2, 2, 3));
    metrics.unreserveResource(user, Resources.createResource(3*GB, 3, 3, 7));
    checkResources(queueSource, 4*GB, 4, 4, 2, 3, 1, 100*GB, 100, 100, 9*GB, 9, 9, 2, 0, 0, 0, 0);
  ...
  public static void checkResources(MetricsSource source, int allocatedMB,
      ...
      long aggreReleasedCtnrs, int availableMB, int availableCores, int availableGPUs ,
      int pendingMB, int pendingCores, int pendingGPUs, int pendingCtnrs,
      int reservedMB, int reservedCores, int reservedGPUs, int reservedCtnrs) {
