...
public class TestCapacitySchedulerMetrics {
  ...
  public void testCSMetrics() throws Exception {
    ...
    final CapacitySchedulerMetrics csMetrics = CapacitySchedulerMetrics.getMetrics();
    Assert.assertNotNull(csMetrics);
    try {
      GenericTestUtils.waitFor(new Supplier<Boolean>() {
        @Override
        public Boolean get() {
          return csMetrics.getNumOfNodeUpdate() == 2;
        }
      }, 100, 3000);
    ...
    try {
      GenericTestUtils.waitFor(new Supplier<Boolean>() {
        @Override
        public Boolean get() {
          return csMetrics.getNumOfNodeUpdate() == 4;
        }
      }, 100, 3000);
    } catch(TimeoutException e) {
      Assert.fail("CS metrics not updated on node-update events.");
