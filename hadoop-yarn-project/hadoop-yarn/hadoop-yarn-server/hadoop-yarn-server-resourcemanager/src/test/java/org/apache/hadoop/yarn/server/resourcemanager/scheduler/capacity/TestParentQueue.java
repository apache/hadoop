...
public class TestParentQueue {
  ...
  public void setUp() throws Exception {
    ...
    when(csContext.getMaximumResourceCapability()).thenReturn(
        Resources.createResource(16*GB, 32, 32, 0xffffffff));
    when(csContext.getClusterResource()).
        thenReturn(Resources.createResource(100 * 16 * GB, 100 * 32, 100 * 32, 0));
  ...
  private FiCaSchedulerApp getMockApplication(int appId, String user) {
    ...
    doReturn(Resources.createResource(0, 0, 0, 0)).when(application).getHeadroom();
    return application;
  }
  ...
  public void testSingleLevelQueues() throws Exception {
    ...
    final Resource clusterResource = 
        Resources.createResource(numNodes * (memoryPerNode*GB),
            numNodes * coresPerNode, numNodes * GPUsPerNode, 0xffffffff);
  ...
  public void testMultiLevelQueues() throws Exception {
    ...
    final Resource clusterResource = 
        Resources.createResource(numNodes * (memoryPerNode*GB), 
            numNodes * coresPerNode, numNodes * GPUsPerNode, 0);
    when(csContext.getNumClusterNodes()).thenReturn(numNodes);
  ...
  public void testOffSwitchSchedulingMultiLevelQueues() throws Exception {
    ...
    final Resource clusterResource = 
        Resources.createResource(numNodes * (memoryPerNode*GB),
            numNodes * coresPerNode, numNodes * GPUsPerNode, (1 << (numNodes * GPUsPerNode)) - 1);
    when(csContext.getNumClusterNodes()).thenReturn(numNodes);
