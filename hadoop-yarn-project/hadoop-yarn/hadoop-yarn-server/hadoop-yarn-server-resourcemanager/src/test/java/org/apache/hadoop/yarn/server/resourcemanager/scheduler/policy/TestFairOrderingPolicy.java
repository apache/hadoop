...
public class TestFairOrderingPolicy {
  ...
  void testSimpleComparison() {
    ...
    Assertions.assertTrue(policy.getComparator().compare(r1, r2) > 0);
  }

  @Test
  void testSizeBasedWeight() {
    FairOrderingPolicy<MockSchedulableEntity> policy =
      new FairOrderingPolicy<MockSchedulableEntity>();
  ...
  @Test
  void testSizeBasedWeightNotAffectAppActivation() throws Exception {
    ...
    Assertions.assertTrue(policy instanceof FairOrderingPolicy);
    Assertions.assertTrue(((FairOrderingPolicy<FiCaSchedulerApp>)policy).getSizeBasedWeight());
    ...
    Assertions.assertEquals(1, lq.getNumActiveApplications());
    Assertions.assertEquals(3, lq.getNumPendingApplications());
  ...
  public void checkIds(Iterator<MockSchedulableEntity> si,
    ...
    for (int i = 0;i < ids.length;i++) {
      Assertions.assertEquals(si.next().getId(),
        ids[i]);
    }
  }

  @Test
  void testOrderingUsingUsedAndPendingResources() {
    FairOrderingPolicy<MockSchedulableEntity> policy =
  ...
  @Test
  void testOrderingUsingAppDemand() {
    FairOrderingPolicy<MockSchedulableEntity> policy =
        new FairOrderingPolicy<MockSchedulableEntity>();
    MockSchedulableEntity r1 = new MockSchedulableEntity();
    MockSchedulableEntity r2 = new MockSchedulableEntity();
  ...
  @Test
  void testRemoveEntitiesWithSizeBasedWeightAsCompletedJobs() {
    FairOrderingPolicy<MockSchedulableEntity> policy =
        new FairOrderingPolicy<MockSchedulableEntity>();
    policy.setSizeBasedWeight(true);
