...
public class TestRMWebServicesNodeLabels extends JerseyTest {
  ...
  private Response get(String path) {
    return getClusterWebResource().path(path).queryParam(QUERY_USER_NAME, userName)
        .request(MediaType.APPLICATION_JSON).get(Response.class);
  }

  private Response get(String path, MultivaluedMap<String, String> queryParams) {
    WebTarget target = getClusterWebResource().path(path).queryParam(QUERY_USER_NAME, userName);
    for (Map.Entry<String, List<String>> set : queryParams.entrySet()) {
      target.queryParam(set.getKey(), set.getValue());
    }
    return target.request(MediaType.APPLICATION_JSON).get(Response.class);
  }

  private Response post(String path, String userName, Object payload,
  ...
  private Response post(String path, String userName, Object payload, Class<?> payloadClass,
      MultivaluedMap<String, String> queryParams) throws Exception {
    WebTarget target = getClusterWebResource().path(path).queryParam(QUERY_USER_NAME, userName);

    for (Map.Entry<String, List<String>> queryParam : queryParams.entrySet()) {
      for (String value : queryParam.getValue()) {
        target.queryParam(queryParam.getKey(), value);
      }
    }

    if (payload != null && payloadClass != null) {
      return target.request(MediaType.APPLICATION_JSON)
          .post(Entity.entity(toJson(payload, payloadClass), MediaType.APPLICATION_JSON),
              Response.class);
    }
  ...
  public void testNodeLabels() throws Exception {
    ...
    response = getLabelsOfNode(NODE_0);
    assertApplicationJsonUtf8Response(response);
    assertNodeLabelsInfoContains(response.readEntity(NodeLabelsInfo.class), Pair.of(LABEL_B, false));
  ...
  private Response replaceNodeToLabels(List<Pair<String, List<String>>> nodeToLabelInfos) throws Exception {
  ...
  private Response getNodeLabelMappingsByLabels(String... labelNames) {
    MultivaluedMap<String, String> params = createMultiValuedMap(labelNames);
  ...
  private static MultivaluedMap<String, String> createMultiValuedMap(String[] labelNames) {
