...
//  }
  ...
  private WebTarget getClusterWebTarget() {
    return webTarget.path(PATH_WS).path(PATH_V1).path(PATH_CLUSTER);
  }

  private Response get(String path) {
    return getClusterWebTarget()
        .path(path)
  ...
  private Response get(String path, MultivaluedMap<String, String> queryParams) {
    WebTarget webTarget = getClusterWebTarget()
        .path(path)
        .queryParam(QUERY_USER_NAME, userName);
  ...
  private Response post(String path, String userName, Object payload,
      Class<?> payloadClass, MultivaluedStringMap queryParams) throws Exception {
    return getClusterWebTarget()
            .path(path)
            .queryParam(QUERY_USER_NAME, userName)
            .request(MediaType.APPLICATION_JSON)
  ...
  private Response getNodeLabelMappingsByLabels(String... labelNames) {
    MultivaluedStringMap params = createMultiValuedMap(labelNames);
  ...
  private static MultivaluedStringMap createMultiValuedMap(String[] labelNames) {
