...
public class TestLeafQueue {
  ...
  public void testRootHasAllNodeLabelsOfItsDescendants() throws IOException {
    CapacitySchedulerConfiguration conf = csConf;
    QueuePath rootChild = root.getChildQueues().get(0).getQueuePathObject();

    conf.setCapacityByLabel(rootChild, "test", 100);
    conf.setCapacityByLabel(rootChild.createNewLeaf(A), "test", 20);
    conf.setCapacityByLabel(rootChild.createNewLeaf(B), "test", 40);
    conf.setCapacityByLabel(rootChild.createNewLeaf(C), "test", 10);
    conf.setCapacityByLabel(rootChild.createNewLeaf(C).createNewLeaf(C1), "test", 100);
    conf.setCapacityByLabel(rootChild.createNewLeaf(D), "test", 30);
    conf.setCapacityByLabel(rootChild.createNewLeaf(E), "test", 0);

    conf.setCapacityByLabel(rootChild, "test2", 100);
    conf.setCapacityByLabel(rootChild.createNewLeaf(A), "test2", 20);
    conf.setCapacityByLabel(rootChild.createNewLeaf(B), "test2", 40);
    conf.setCapacityByLabel(rootChild.createNewLeaf(C), "test2", 10);
    conf.setCapacityByLabel(rootChild.createNewLeaf(C).createNewLeaf(C1), "test2", 100);
    conf.setCapacityByLabel(rootChild.createNewLeaf(D), "test2", 30);
    conf.setCapacityByLabel(rootChild.createNewLeaf(E), "test2", 0);
