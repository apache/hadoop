...
public class TestClientRMService {
  ...
  public void testGetApplications() throws Exception {
    ...
    tagSet = new HashSet<>(Collections.singletonList(tags.get(0)));
    request.setApplicationTags(tagSet);
    assertEquals("Incorrect number of matching tags", 3,
        rmService.getApplications(request).getApplicationList().size());

    tagSet = new HashSet<>(Collections.singletonList(tags.get(1)));
    request.setApplicationTags(tagSet);
    assertEquals("Incorrect number of matching tags", 2,
        rmService.getApplications(request).getApplicationList().size());

    tagSet = new HashSet<>(Collections.singletonList(tags.get(2)));
    request.setApplicationTags(tagSet);
