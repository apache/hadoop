...
public class TestResourceUsage {
  ...
  private void internalTestModifyAndRead(String label) throws Exception {
    ...
    inc(usage, suffix, Resource.newInstance(1, 1, 1, 1), label);
    check(1, 1, 1, get(usage, suffix, label));

    // Set 2,2
    set(usage, suffix, Resource.newInstance(2, 2, 2, 3), label);
    check(2, 2, 2, get(usage, suffix, label));

    // dec 2,2
    dec(usage, suffix, Resource.newInstance(2, 2, 2, 3), label);
