...
public class TestNodeLabelContainerAllocation {
  ...
  public void testQueueMetricsWithMixedLabels() throws Exception {
    ...
    // There is a race condition (fixed in trunk) in AsyncDispatcher where
    // dispatcher thread sets drained to true between handler setting drained
    // to false and enqueueing event. See YARN-3878, YARN-5436, and YARN-5375.
    // In this test, NODE_LABEL_UPDATE event is not processed prior to reaching
    // this part of the code, which results in incorrect available MB.
    // For now, we will get around it by simply waiting 5 or so seconds.
    Thread.sleep(5000);
