...
|`yarn.federation.policy-manager-params` | `<binary>` | The payload that configures the policy. In our example a set of weights for router and amrmproxy policies. This is typically generated by serializing a policymanager that has been configured programmatically, or by populating the state-store with the .json serialized form of it.  Below is the example: <br/> {"routerPolicyWeights": {"entry": [{"key": {"id": "sc1"},"value": "0.07472054"}]},"amrmPolicyWeights": {"entry": [{"key":{"id": "sc1"},"value": "0.07472054"}, {"key":{"id": "sc16"},"value": "0.14857134"}]},"headroomAlpha": "0.0"} |
