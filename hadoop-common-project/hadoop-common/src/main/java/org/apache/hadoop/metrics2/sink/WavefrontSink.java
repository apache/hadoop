...
/**
 * A metrics sink that writes to a Wavefront server
 */
    ...
    public void putMetrics(MetricsRecord record) {
        ...
        for (MetricsTag tag : record.tags()) {
            if (tag.value() != null) {
                ...
                // if point tag contains key of hostname, and source is null or empty, use it instead
                if((metricsSource == null || metricsSource.trim().length() == 0) && tag.name().equalsIgnoreCase("hostname")) metricsSource = tag.value();
            }
        }
