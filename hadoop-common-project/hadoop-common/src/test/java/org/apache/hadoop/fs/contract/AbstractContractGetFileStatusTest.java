...
,         toListThroughNextCallsAlone(
            fs.listFiles(tree.getBasePath(), true)).size(), "Size of status list through next() calls");
  }
  ...
  public void testListFilesFile() throws Throwable {
    ...
    List<LocatedFileStatus> statusList2 = toListThroughNextCallsAlone(
        getFileSystem().listFiles(f, false));
    assertEquals(
       1, statusList2.size(), "size of file list returned through next() calls");
  ...
  private FileStatus[] verifyListStatus(int expected,
    ...
    assertEquals(
       expected, result.length, "length of listStatus(" + path + ", " + filter + " ) " +
  ...
  private List<LocatedFileStatus> verifyListLocatedStatus(ExtendedFilterFS xfs,
    ...
    assertEquals(
       expected, result.size(), "length of listLocatedStatus(" + path + ", " + filter + " )");
    return result;
  }
