...
public abstract class ClientBaseWithFixes extends ZKTestCase {
  ...
  static {
    ...
    protected static ZooKeeperServer getServer(ServerCnxnFactory fac) {
        try {
            java.lang.reflect.Field f = ServerCnxnFactory.class.getDeclaredField("zkServer");
            f.setAccessible(true);
            return (ZooKeeperServer) f.get(fac);
        } catch (Exception e) {
            throw new RuntimeException("Failed to get ZooKeeperServer from ServerCnxnFactory", e);
        }
    }
