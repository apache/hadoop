...
public class TestMapFile {
  ...
  public void setup() throws Exception {
    ...
    if (fs.exists(TEST_DIR) && !fs.delete(TEST_DIR, true)) {
      Assertions.fail("Can't clean up test root dir");
    }
    fs.mkdirs(TEST_DIR);
  }
  ...
  public void testRenameWithException() {
    ...
    try {
      ...
      assertEquals(
         ex.getMessage(), ERROR_MESSAGE, "testRenameWithException invalid IOExceptionMessage !!!");
    } finally {
      IOUtils.cleanupWithLogger(LOG, writer);
    }
  }
  ...
  public void testRenameWithFalse() {
    ...
    try {
      ...
      assertTrue(ex
          .getMessage().startsWith(ERROR_MESSAGE), "testRenameWithFalse invalid IOExceptionMessage error !!!");
    } finally {
      IOUtils.cleanupWithLogger(LOG, writer);
    }
  }
  ...
  public void testWriteWithFailDirCreation() {
    ...
    try {
      ...
      assertTrue(ex.getMessage()
          .startsWith(ERROR_MESSAGE), "testWriteWithFailDirCreation ex error !!!");
    } finally {
      IOUtils.cleanupWithLogger(LOG, writer);
    }
  }
  ...
  public void testOnFinalKey() {
    ...
    try {
      ...
      assertEquals(expectedKey, new IntWritable(
          9), "testOnFinalKey not same !!!");
    } catch (IOException ex) {
  ...
  public void testKeyValueClasses() {
    ...
    try {
      createWriter("testKeyValueClasses.mapfile", IntWritable.class, Text.class)
        .close();
      assertNotNull(
         MapFile.Writer.keyClass(keyClass), "writer key class null error !!!");
      assertNotNull(
         MapFile.Writer.valueClass(valueClass), "writer value class null error !!!");
    } catch (IOException ex) {
      fail(ex.getMessage());
    }
  }
  ...
  public void testReaderKeyIteration() {
    ...
    try {
      ...
      assertTrue(
         reader.seek(new IntWritable(SIZE / 2)), "reader seek error !!!");
      assertFalse(
         reader.seek(new IntWritable(SIZE * 2)), "reader seek error !!!");
    } catch (IOException ex) {
  ...
  public void testFix() {
    ...
    try {
      ...
      if (isDeleted)
        assertTrue(
           MapFile.fix(fs, dir, IntWritable.class, Text.class, true, conf) == PAIR_SIZE, "testFix error !!!");
    } catch (Exception ex) {
      fail("testFix error !!!");
    } finally {
      IOUtils.cleanupWithLogger(LOG, writer);
...
,                    MapFile.fix(fs, dir, IntWritable.class, Text.class,
                               false, conf), "No of valid MapFile entries wrong");
      ...
      assertEquals(
                  0, notFound, "With MapFile.fix-ed index, could not get entries # ");
    } finally {
  ...
  public void testPathExplosionWriterCreation() {
    ...
    try {
      ...
      assertEquals(
         ex.getMessage(), TEST_ERROR_MESSAGE, "testPathExplosionWriterCreation ex message error !!!");
    } catch (Exception e) {
  ...
  public void testMerge() throws Exception {
    ...
    try (MapFile.Reader reader = createReader(TEST_METHOD_KEY,
      ...
      while (i++ < ITERATIONS) {
        ...
        while (reader.next(key, value)) {
          assertTrue(
             prev.get() <= key.get(), "Next key should be always equal or more");
    ...
    for (int j = 0; j < in.length; j++) {
      Path path = in[j];
      assertFalse(
         path.getFileSystem(conf).exists(path), "inputs should be deleted");
    }
  }
}
