...
final class OpRunTimeTopN {
  ...
  static final String UNKNOWN_CONTAINER_ID = "unknown-container";
  ...
  static final class Sample implements Comparable<Sample> {
    ...
    Sample(long startTime, long endTime, String containerId) {
      this.startTime = Math.max(0L, startTime);
      this.endTime = Math.max(this.startTime, endTime);
      this.duration = this.endTime - this.startTime;
      this.containerId = normalizeContainerId(containerId);
    }
  ...
  static final class Tracker {
    ...
    Tracker(int limit, String containerId) {
      this.limit = limit;
      this.containerId = normalizeContainerId(containerId);
    ...
    void record(String opType, long startTime, long endTime) {
      ...
      addSample(queue, new Sample(startTime, endTime, containerId), limit);
    }
    ...
    void addEncodedSamples(String encoded) {
      ...
      for (String entry : entries) {
        ...
        try {
          ...
          String containerId = UNKNOWN_CONTAINER_ID;
          if (fields.length == 4) {
            containerId = fields[3].trim();
          }
          addSample(heap, new Sample(startTime, endTime, containerId), limit);
  ...
  private static String encodeSamples(Collection<Sample> samples) {
    ...
    for (Sample sample : sorted) {
      ...
      builder.append(FIELD_DELIM);
      builder.append(sample.containerId);
    }
    return builder.toString();
  }

  static String encodeSingle(long startTime, long endTime) {
    return encodeSamples(Collections.singletonList(
        new Sample(startTime, endTime, UNKNOWN_CONTAINER_ID)));
  }
  ...
  private static String normalizeContainerId(String containerId) {
    ...
    return trimmed.isEmpty() ? UNKNOWN_CONTAINER_ID : trimmed;
  }
}
