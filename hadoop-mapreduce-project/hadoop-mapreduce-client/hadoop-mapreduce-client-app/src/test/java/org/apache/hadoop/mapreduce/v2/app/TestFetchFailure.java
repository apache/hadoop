...
public class TestFetchFailure {
  ...
  void testFetchFailure() throws Exception {
    ...
    assertEquals(TaskAttemptCompletionEventStatus.SUCCEEDED, events[0].getStatus(), "Event status not correct");
    ...
    assertEquals(TaskAttemptState.FAILED, mapAttempt1.getState(), "Map TaskAttempt state not correct");

    assertEquals(2, mapTask.getAttempts().size(), "Num attempts in Map Task not correct");
    ...
    assertEquals(TaskAttemptCompletionEventStatus.OBSOLETE, events[0].getStatus(), "Event status not correct");

    events = job.getTaskAttemptCompletionEvents(0, 100);
    assertEquals(4, events.length, "Num completion events not correct");
    assertEquals(mapAttempt1.getID(), events[0].getAttemptId(), "Event map attempt id not correct");
    assertEquals(mapAttempt1.getID(), events[1].getAttemptId(), "Event map attempt id not correct");
    assertEquals(mapAttempt2.getID(), events[2].getAttemptId(), "Event map attempt id not correct");
    assertEquals(reduceAttempt.getID(), events[3].getAttemptId(), "Event redude attempt id not correct");
    assertEquals(TaskAttemptCompletionEventStatus.OBSOLETE, events[0].getStatus(), "Event status not correct for map attempt1");
    assertEquals(TaskAttemptCompletionEventStatus.FAILED, events[1].getStatus(), "Event status not correct for map attempt1");
    assertEquals(TaskAttemptCompletionEventStatus.SUCCEEDED, events[2].getStatus(), "Event status not correct for map attempt2");
    assertEquals(TaskAttemptCompletionEventStatus.SUCCEEDED, events[3].getStatus(), "Event status not correct for reduce attempt1");
    ...
    assertArrayEquals(Arrays.copyOfRange(convertedEvents, 0, 2), mapEvents, "Unexpected map events");
    mapEvents = job.getMapAttemptCompletionEvents(2, 200);
    assertEquals(1, mapEvents.length, "Incorrect number of map events");
    assertEquals(convertedEvents[2],
        mapEvents[0],
  ...
  void testFetchFailureMultipleReduces() throws Exception {
    ...
    assertEquals(mapAttempt1.getID(), events[1].getAttemptId(), "Event map attempt id not correct");
    assertEquals(mapAttempt2.getID(), events[2].getAttemptId(), "Event map attempt id not correct");
    assertEquals(reduceAttempt.getID(), events[3].getAttemptId(), "Event reduce attempt id not correct");
    assertEquals(TaskAttemptCompletionEventStatus.OBSOLETE, events[0].getStatus(), "Event status not correct for map attempt1");
    assertEquals(TaskAttemptCompletionEventStatus.FAILED, events[1].getStatus(), "Event status not correct for map attempt1");
    assertEquals(TaskAttemptCompletionEventStatus.SUCCEEDED, events[2].getStatus(), "Event status not correct for map attempt2");
    assertEquals(TaskAttemptCompletionEventStatus.SUCCEEDED, events[3].getStatus(), "Event status not correct for reduce attempt1");
