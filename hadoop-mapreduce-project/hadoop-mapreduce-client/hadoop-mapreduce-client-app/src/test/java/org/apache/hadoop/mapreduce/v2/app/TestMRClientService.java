...
public class TestMRClientService {
  ...
  void test() throws Exception {
    ...
    assertNotNull(proxy.getCounters(gcRequest).getCounters(),
        "Counters is null");
    ...
    assertEquals(diagnostic1, diag.get(0).toString(), "Diag 1 not correct");
  ...
  void testViewAclOnlyCannotModify() throws Exception {
    ...
    assertTrue(job.checkAccess(viewOnlyUser, JobACL.VIEW_JOB),
        "viewonlyuser cannot view job");
    assertFalse(job.checkAccess(viewOnlyUser, JobACL.MODIFY_JOB),
  ...
  private void verifyTaskAttemptReport(TaskAttemptReport tar) {
    ...
    assertEquals(1, tar.getContainerId().getApplicationAttemptId()
        .getAttemptId());
  }
