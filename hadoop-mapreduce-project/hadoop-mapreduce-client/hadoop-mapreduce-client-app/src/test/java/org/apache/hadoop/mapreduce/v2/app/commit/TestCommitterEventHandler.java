...
public class TestCommitterEventHandler {
  ...
  void testCommitWindow() throws Exception {
    ...
    assertEquals(1, rmhh.getNumCallbacks(), "committer did not register a heartbeat callback");
    verify(committer, never()).commitJob(any(JobContext.class));
    assertEquals(0, jeh.numCommitCompletedEvents, "committer should not have committed");
    ...
    assertEquals(1, jeh.numCommitCompletedEvents, "committer did not complete commit after RM hearbeat");
    ...
    assertEquals(2, jeh.numCommitCompletedEvents, "committer did not commit");
