 ...
 */
public class HdfsCompatibility extends Configured implements Tool {
  private static final Logger LOG =
      LoggerFactory.getLogger(HdfsCompatibility.class);

  private static final String DESCRIPTION = "hdfs compatibility" +
      " -uri <uri> [-suite <suite>]:\n" +
      "\tTrigger a compatibility check progress for a specific" +
      " Storage System implementation.\n" +
      "\tA compatibility report is generated after the process finished," +
      ...
      "\t-suite is optional for limiting the check to a subset." +
      " For example, 'shell' means command line only.";
  ...
  HdfsCompatibility(Configuration conf) {
    this(conf, System.out, System.err);
  }

  HdfsCompatibility(Configuration conf, PrintStream out, PrintStream err) {
    super(conf);
    this.out = out;
    this.err = err;
  }
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new HdfsCompatibility(new Configuration()), args);
    System.exit(res);
  }
}
