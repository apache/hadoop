...
public class TestCopyOutputFormat {
  ...
  public void testSetCommitDirectory() {
    try {
      ...
      Assertions.assertEquals(null, CopyOutputFormat.getCommitDirectory(job));
      ...
      Assertions.assertEquals(directory, CopyOutputFormat.getCommitDirectory(job));
      Assertions.assertEquals(directory.toString(), job.getConfiguration().
          get(DistCpConstants.CONF_LABEL_TARGET_FINAL_PATH));
    } catch (IOException e) {
      LOG.error("Exception encountered while running test", e);
      Assertions.fail("Failed while testing for set Commit Directory");
    }
  }
  ...
  public void testSetWorkingDirectory() {
    try {
      Job job = Job.getInstance(new Configuration());
      Assertions.assertEquals(null, CopyOutputFormat.getWorkingDirectory(job));
      ...
      Assertions.assertEquals(directory, CopyOutputFormat.getWorkingDirectory(job));
      Assertions.assertEquals(directory.toString(), job.getConfiguration().
          get(DistCpConstants.CONF_LABEL_TARGET_WORK_PATH));
    } catch (IOException e) {
      LOG.error("Exception encountered while running test", e);
      Assertions.fail("Failed while testing for set Working Directory");
    }
  }
  ...
  public void testGetOutputCommitter() {
    try {
      ...
      Assertions.fail("Unable to get output committer");
    }
  }
  ...
  public void testCheckOutputSpecs() {
    try {
      ...
      try {
        ...
        Assertions.fail("No checking for invalid work/commit path");
      ...
      try {
        JobContext context = new JobContextImpl(job.getConfiguration(), jobID);
        outputFormat.checkOutputSpecs(context);
        Assertions.fail("No checking for invalid commit path");
      } catch (IllegalStateException ignore) { }
      ...
      try {
        JobContext context = new JobContextImpl(job.getConfiguration(), jobID);
        outputFormat.checkOutputSpecs(context);
        Assertions.fail("No checking for invalid work path");
      } catch (IllegalStateException ignore) { }
      ...
      try {
        ...
        Assertions.fail("Output spec check failed.");
      }

    } catch (IOException e) {
      LOG.error("Exception encountered while testing checkoutput specs", e);
      Assertions.fail("Checkoutput Spec failure");
