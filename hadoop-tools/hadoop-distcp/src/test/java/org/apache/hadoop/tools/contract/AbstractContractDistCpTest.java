...
public abstract class AbstractContractDistCpTest
  ...
  void assertCounterInRange(Job job, Enum<?> counter, long min, long max)
    ...
    if (min >= 0) {
      assertTrue(
         value >= min, description + " too below minimum " + min);
    }
    if (max >= 0) {
      assertTrue(
         value <= max, description + " above maximum " + max);
    }
  }
  ...
  public void testTrackDeepDirectoryStructureToRemote() throws Exception {
    ...
    assertTrue(
       sourceFiles.containsValue(inputFileNew1), "No " + outputFileNew1 + " in source listing");
    assertTrue(
       targetFiles.containsValue(outputFileNew1), "No " + outputFileNew1 + " in target listing");
    assertTrue(
       targetFiles.containsValue(outputSubDir4), "No " + outputSubDir4 + " in target listing");
    assertFalse(
       sourceFiles.containsValue(inputSubDir4), "Found " + inputSubDir4 + " in source listing");
  ...
  public void testSetJobId() throws Exception {
    ...
    assertNotNull(
       conf.get(CONF_LABEL_DISTCP_JOB_ID), "DistCp job id isn't set");
  }
