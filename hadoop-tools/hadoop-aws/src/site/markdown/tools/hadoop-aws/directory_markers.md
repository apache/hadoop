...
# Altering the S3A Directory Marker Retention Policy

## <a name="warning"></a> Critical: this is not backwards compatible!
...
Older versions of Hadoop will

Versions of Hadoop which are incompatible with other marker retention policies


|  Branch | Incompatible releases | Future Fix Planned? |
|---------|-----------------------|--------------------|
| Hadoop 2.x | All releases | WONTFIX |
| Hadoop 3.0 | All releases |  WONTFIX |
| Hadoop 3.1 | All releases |  Yes |
| Hadoop 3.2 | All releases |  Yes |
| Hadoop 3.3 | 3.3.0 |  Yes |
...
2. You know all applications which read data from the bucket are listed as compatible.

### <a name="backups"></a> Applications backing up data MUST ALSO be based on compatible products.
...
Applications using an incompatible version of the S3A connector will mistake directories
containing data for empty directories. This means that
...
If there are two objects Cand `a/b/file2` then S3A pretends that there is a
...
exists. They also assume that after files in a directory are delted, the
...
all parents. For example, if a file `/a/b/file1` is created, a `DELETE` request
containing the keys `/a/` and `/a/b/` is issued. If no marker exists, this is
nominally a no-op.
...
because they know how their file uses


###  Hadoop 3.3.1+: marker deletion is now optional


## <a name=""></a> Controlling marker retention with `fs.s3a.directory.marker.retention` 
...
##  <a name=""></a> The marker tool: 'hadoop s3guard marker'
...
The `markers clean` command will clean the directory tree
