...
public class TestS3AAWSCredentialsProvider extends AbstractS3ATestBase {
  ...
  public void testAuthenticationContainsProbes() {
    Configuration conf = new Configuration(false);
    assertFalse(
       authenticationContains(conf, AssumedRoleCredentialProvider.NAME), "found AssumedRoleCredentialProvider");

    conf.set(AWS_CREDENTIALS_PROVIDER, AssumedRoleCredentialProvider.NAME);
    assertTrue(
       authenticationContains(conf, AssumedRoleCredentialProvider.NAME), "didn't find AssumedRoleCredentialProvider");
  }
...
,         retryPolicy.shouldRetry(noAuth, 0, 0, true).action, "Expected no retry on auth failure");
    ...
    try {
      ...
      assertEquals(
         RetryPolicy.RetryAction.FAIL.action
,           retryPolicy.shouldRetry(expected, 0, 0, true).action, "Expected no retry on AccessDeniedException");
    }
  ...
  public void testRefCounting() throws Throwable {
    AWSCredentialProviderList providers
        = new AWSCredentialProviderList();
    assertEquals(
       1, providers.getRefCount(), "Ref count for " + providers);
    AWSCredentialProviderList replicate = providers.share();
    assertEquals(providers, replicate);
    assertEquals(
       2, providers.getRefCount(), "Ref count after replication for " + providers);
    assertFalse(providers.isClosed(), "Was closed " + providers);
    providers.close();
    assertFalse(providers.isClosed(), "Was closed " + providers);
    assertEquals(
       1, providers.getRefCount(), "Ref count after close() for " + providers);

    // this should now close it
    providers.close();
    assertTrue(providers.isClosed(), "Was not closed " + providers);
    assertEquals(
       0, providers.getRefCount(), "Ref count after close() for " + providers);
    assertEquals(
       0, providers.getRefCount(), "Ref count after second close() for " + providers);
  ...
  public void testConcurrentAuthentication() throws Throwable {
    ...
    try {
      assertFalse(
         ...
         provider.isInitialized(), "Provider not initialized. isInitialized should be false");
      assertFalse(
      
         provider.hasCredentials(), "Provider not initialized. hasCredentials should be false");
    ...
    assertTrue(
       ...
       provider.isInitialized(), "Provider initialized without errors. isInitialized should be true");
    assertTrue(
    
       provider.hasCredentials(), "Provider initialized without errors. hasCredentials should be true");
    if (provider.getInitializationException() != null) {
      throw new AssertionError(
  ...
  public void testConcurrentAuthenticationError() throws Throwable {
    ...
    assertTrue(
       ...
       provider.isInitialized(), "Provider initialization failed. isInitialized should be true");
    assertFalse(
    
       provider.hasCredentials(), "Provider initialization failed. hasCredentials should be false");
    assertTrue(
    
       provider.getInitializationException().getMessage().contains("expected error"), "Provider initialization failed. getInitializationException should contain the error");
  }
