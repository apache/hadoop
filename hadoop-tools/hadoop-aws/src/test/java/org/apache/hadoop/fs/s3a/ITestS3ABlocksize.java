...
public class ITestS3ABlocksize extends AbstractS3ATestBase {
  ...
  public void testBlockSize() throws Exception {
    ...
    assertEquals(
       S3AFileSystem.DEFAULT_BLOCKSIZE, defaultBlockSize, "incorrect blocksize");
    ...
    assertEquals(
       newBlockSize
,         fileStatus.getBlockSize(), "Double default block size in stat(): " + fileStatus);
    ...
    for (FileStatus stat : listing) {
      ...
      if (file.equals(stat.getPath())) {
        ...
        assertEquals(
           newBlockSize
,             stat.getBlockSize(), "Double default block size in ls(): " + stat);
      }
    }
  ...
  public void testRootFileStatusHasBlocksize() throws Throwable {
    ...
    assertTrue(
       status.getBlockSize() >= 0, "Invalid root blocksize");
  }
