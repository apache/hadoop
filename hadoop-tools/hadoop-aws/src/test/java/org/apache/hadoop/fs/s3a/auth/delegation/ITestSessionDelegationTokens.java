...
public class ITestSessionDelegationTokens extends AbstractDelegationIT {
  ...
  public void testCanonicalization() throws Throwable {
    ...
    assertEquals(
       0, fs.getDefaultPort(), "Default port has changed");
    URI uri = fs.getCanonicalUri();
    String service = fs.getCanonicalServiceName();
    assertEquals(
       uri, new URI(service), "canonical URI and service name mismatch");
  }
  ...
  public void testSaveLoadTokens() throws Throwable {
    ...
    assertEquals(
       origIdentifier.getOrigin(), decoded.getOrigin(), "Origin in " + decoded);
    assertEquals(
       origIdentifier.getExpiryTime(), decoded.getExpiryTime(), "Expiry time");
    assertEquals(
       encryptionSecrets, decoded.getEncryptionSecrets(), "Encryption Secrets");
  }
  ...
  public void testCreateAndUseDT() throws Throwable {
    ...
    assertNull(
       delegationTokens.selectTokenFromFSOwner(), "Current User has delegation token");
    ...
    try(S3ADelegationTokens dt2 = instantiateDTSupport(getConfiguration())) {
      ...
      assertThat(userAgentField).
          contains(issued.getUuid()).
  ...
  protected AbstractS3ATokenIdentifier verifyCredentialPropagation(
    ...
    try(S3ADelegationTokens delegationTokens2 = new S3ADelegationTokens()) {
      ...
      assertTrue(
         boundId.getOrigin()
              .contains(CREDENTIALS_CONVERTED_TO_DELEGATION_TOKEN), "Origin in " + boundId);
      return boundId;
    }
  }
  ...
  public void testDBindingReentrancyLock() throws Throwable {
    ...
    delegation.start();
    assertFalse(
       delegation.isBoundToDT(), "Delegation is bound to a DT: " + delegation);
  }
