...
public abstract class AbstractITCommitProtocol extends AbstractCommitITest {
  ...
  public void testRecoveryAndCleanup() throws Exception {
    ...
    assertNotNull(
       committer.getWorkPath(), "null workPath in committer " + committer);
    assertNotNull(
       committer.getOutputPath(), "null outputPath in committer " + committer);
    ...
    assertFalse(
       committer2.isRecoverySupported(), "recoverySupported in " + committer2);
...
,         job2TaskAttempt0Path, "Task attempt paths must differ");
  ...
  public void testCommitLifecycle() throws Exception {
    ...
    assertTrue(
       committer.needsTaskCommit(tContext), "No files to commit were found by " + committer);
    commitTask(committer, tContext);
  ...
  public void assertJobAbortCleanedUp(JobData jobData) throws Exception {
    ...
    try {
      ...
      assertArrayEquals(
         new FileStatus[0], children, "Output directory not empty " + ls(outDir));
  ...
  public void testOutputFormatIntegration() throws Throwable {
    ...
    assertTrue(
       committer.needsTaskCommit(tContext), "Committer does not have data to commit " + committer);
    commitTask(committer, tContext);
...
,           committer2.getUUID(), "job UUIDs");
  ...
  public void testParallelJobsToSameDestination() throws Throwable {
    ...
    assertNotEquals(
       job1Output, job2Output, "Job output file filenames must be different");
