...
public abstract class ITestS3AClientSideEncryption extends AbstractS3ATestBase {
  ...
  public void testBigFilePutAndGet() throws IOException {
    ...
    try (FSDataInputStream in = fs.open(filePath)) {
      ...
      assertEquals(
         offsetSeek, in.read(), "Byte at a specific position not equal to actual byte");
      in.seek(0);
      assertEquals(
         'a', in.read(), "Byte at a specific position not equal to actual byte");
      ...
      assertEquals(
         byteBeforeBlockEnd - 1, in.read(), "Byte before multipart block end mismatch");
      assertEquals(
         byteBeforeBlockEnd, in.read(), "Byte at multipart end mismatch");
      assertEquals(
         byteBeforeBlockEnd + 1, in.read(), "Byte after multipart end mismatch");

      // Verify end of file seek read.
      in.seek(BIG_FILE_SIZE + 1);
      assertEquals(
         -1, in.read(), "Byte at eof mismatch");
  ...
  public void testEncryptionEnabledAndDisabledFS() throws Exception {
    ...
    try (FSDataInputStream in = cseDisabledFS.open(encryptedFilePath)) {
      ...
      assertNotEquals(1
,           unEncryptedFSFileStatus.getLen(), "Mismatch in content length");
