...
public class TestV1CredentialsProvider {
  ...
  private static void assertCredentialProviders(
    ...
    for (int i = 0; i < expectedClasses.size(); ++i) {
      ...
      assertNotNull(
      provider, String.format("At position %d, expected class is %s, but found null.",
              i, expectedClass));
      assertTrue(
      
         expectedClass.isAssignableFrom(provider.getClass()), String.format("At position %d, expected class is %s, but found %s.",
