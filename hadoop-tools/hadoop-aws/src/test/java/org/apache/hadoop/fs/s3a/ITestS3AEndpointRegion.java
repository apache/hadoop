...
public class ITestS3AEndpointRegion extends AbstractS3ATestBase {
   ...
   */
  private static final AtomicReference<String> expectedMessage = new AtomicReference<>();
  /**
  ...
  private static final class RegionInterceptor implements ExecutionInterceptor {
    ...
    public void beforeExecution(Context.BeforeExecution context,
      ...
      final String reg = executionAttributes.getAttribute(AwsExecutionAttribute.AWS_REGION).toString();
      ...
      if (endpoint != null && !endpoint.endsWith(CENTRAL_ENDPOINT)) {
        Assertions.assertThat(
                endpointOveridden)
            .describedAs("Endpoint not overridden in %s. Client Config=%s", state, expectedMessage.get())
            .isTrue();

        Assertions.assertThat(
                clientEndpoint)
            .describedAs("There is an endpoint mismatch in %s. Client Config=%s"
                , state, expectedMessage.get())
