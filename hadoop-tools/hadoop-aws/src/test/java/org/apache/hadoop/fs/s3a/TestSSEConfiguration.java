...
public class TestSSEConfiguration extends Assert {
  ...
  private Configuration buildConf(String algorithm, String key) {
    ...
    if (algorithm != null) {
      conf.set(S3_ENCRYPTION_ALGORITHM, algorithm);
    } else {
      conf.unset(S3_ENCRYPTION_ALGORITHM);
    }
    if (key != null) {
      conf.set(S3_ENCRYPTION_KEY, key);
    } else {
