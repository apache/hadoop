...
public class ITestS3APrefetchingInputStream extends AbstractS3ACostTest {
  ...
  public void testStatusProbesAfterClosingStream() throws Throwable {
    ...
    assertNotEquals(0
,         pos, "Position retrieved from prefetching input stream should be greater than 0");
    ...
    assertNotEquals(0
,         newPos, "Position retrieved from prefetching input stream should be greater than 0");

    // compare status probes after closing of the stream with status probes done before
    // closing the stream
    assertEquals(pos
,         newPos, "Position retrieved through stream before and after closing should match");
    assertEquals(ioStats
,         newIoStats, "IO stats retrieved through stream before and after closing should match");
    assertEquals(
       inputStreamStatistics, newInputStreamStatistics, "Stream stats retrieved through stream before and after closing should match");

    assertFalse(in.seekToNewSource(10), "seekToNewSource() not supported with prefetch");
  }
