...
public class ITestS3AUnbuffer extends AbstractS3ATestBase {
  ...
  public void testUnbuffer() throws IOException {
    ...
    try (FSDataInputStream inputStream = getFileSystem().open(dest)) {
      ...
      assertTrue(
         iostats.aggregate(inputStream.getIOStatistics()), "No IOstatistics from " + inputStream);
  ...
  public void testUnbufferStreamStatistics() throws IOException {
    ...
    assertEquals(
       expectedFinalBytesRead
,             streamStatistics.getBytesRead(), "S3AInputStream statistics were not updated properly in "
        + streamStatsStr);
  }
...
,             inputStream.read(new byte[bytesToRead]), "S3AInputStream#read did not read the correct number of " +
                    "bytes");
  }
}
