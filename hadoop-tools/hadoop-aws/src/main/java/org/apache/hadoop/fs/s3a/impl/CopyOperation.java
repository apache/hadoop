...
public class CopyOperation extends ExecutingStoreOperation<Long> {
  ...
  private void verifyCopyConditions()
      throws FileAlreadyExistsException, IllegalArgumentException, IOException {
    Path srcPath = srcStatus.getPath();
    Path dstPath = new Path(dstURI);
    FileSystem dstFS = dstPath.getFileSystem(getStoreContext().getConfiguration());
    try {
      FileStatus dstStatus = dstFS.getFileStatus(dstPath);
      if (dstStatus.isFile()) {
        throw new FileAlreadyExistsException(dstPath.toUri() + " already exists");
      }
      if (dstStatus.isDirectory()) {
        throw new IllegalArgumentException(dstURI + " does not contain filename");
      }
    } catch (FileNotFoundException fnfe) {
      // destination file is not present. safe.
    }
    if (!dstFS.exists(dstPath.getParent())) {
      throw new IllegalArgumentException(dstPath.getParent() + " not present");
    }
    if (srcStatus.isDirectory()) {
      throw new IllegalArgumentException(srcPath.toUri() + " can not be a directory");
    }
  }
  ...
  protected void copyFile() throws IOException {
    ...
    LOG.debug("copy: copying file {} to {}", srcStatus.getPath().toUri(), dstURI);
    // copy
    try (DurationInfo ignored = new DurationInfo(LOG, false,
        "Copy file from %s to %s (length=%d)", srcURI, dstURI.toString(), srcStatus.getLen())) {
