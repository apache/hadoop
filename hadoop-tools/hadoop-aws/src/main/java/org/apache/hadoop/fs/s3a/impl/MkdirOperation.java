 ...
 * Optimized for finding directories.
 */
public class MkdirOperation extends ExecutingStoreOperation<Boolean> {
  ...
  public Boolean execute() throws IOException {
    ...
    FileStatus fileStatus = getFileStatusExpectingDir(dir);
    if (fileStatus != null) {
    ...
    while (fPart != null && !fPart.isRoot()) {
      fileStatus = getFileStatusExpectingDir(fPart);
      if (fileStatus == null) {
  ...
  private S3AFileStatus probeDirStatusOrNull(final Path path,
      final Set<StatusProbeEnum> probes) throws IOException {
    try {
      return callbacks.probeDirStatus(path, probes);
    } catch (FileNotFoundException fnfe) {
      return null;
    }
  }

  private S3AFileStatus getFileStatusExpectingDir(final Path path)
      throws IOException {
    S3AFileStatus status = probeDirStatusOrNull(path,
        StatusProbeEnum.DIRECTORIES);
    if (status == null) {
      status = probeDirStatusOrNull(path,
  ...
  public interface MkdirCallbacks {
     ...
     */
    @Retries.RetryTranslated
    S3AFileStatus probeDirStatus(final Path path,
        final Set<StatusProbeEnum> probes) throws IOException;
     ...
     */
    @Retries.RetryTranslated
    void createFakeDirectory(final String objectName) throws IOException;
  }
}
