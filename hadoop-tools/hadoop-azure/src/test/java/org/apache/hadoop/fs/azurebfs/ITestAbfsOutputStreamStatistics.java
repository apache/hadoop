...
,           abfsOutputStreamStatisticsForUploadBytes.getBytesToUpload(), "Mismatch in bytes to upload");
      ...
      assertEquals(
         testBytesToUpload.getBytes().length
,           abfsOutputStreamStatisticsForUploadBytes.getBytesUploadSuccessful(), "Mismatch in successful bytes uploaded");
...
,           abfsOutputStreamStatistics.getBytesToUpload(), "Mismatch in bytes to upload");
      ...
      assertEquals(
         OPERATIONS * (testBytesToUpload.getBytes().length)
,           abfsOutputStreamStatistics.getBytesUploadSuccessful(), "Mismatch in successful bytes uploaded");
...
,           abfsOutputStreamStatistics.getQueueShrunkOps(), "Mismatch in queue shrunk operations");
  ...
  public void testAbfsOutputStreamWriteBuffer() throws IOException {
    ...
    try (AbfsOutputStream outForOneOp = createAbfsOutputStreamWithFlushEnabled(
      ...
      assertEquals(0
,           abfsOutputStreamStatistics.getWriteCurrentBufferOperations(), "Mismatch in write current buffer operations");
