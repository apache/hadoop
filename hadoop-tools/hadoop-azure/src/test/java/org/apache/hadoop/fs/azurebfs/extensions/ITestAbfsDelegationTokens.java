...
public class ITestAbfsDelegationTokens extends AbstractAbfsIntegrationTest {
  ...
  public void testCanonicalization() throws Throwable {
    ...
    assertEquals(
       getFilesystemURI(), new URI(service), "canonical URI and service name mismatch");
  }
  ...
  public void testDefaultCanonicalization() throws Throwable {
    ...
    assertEquals(
       getDefaultServiceName(fs), getCanonicalServiceName(), "canonicalServiceName is not the default");
  }
...
,         credentials.numberOfTokens(), "Number of collected tokens");
  ...
  public void testRequestTokenDefault() throws Throwable {
    ...
    AzureBlobFileSystem fs = getFileSystem();
    assertEquals(
       getDefaultServiceName(fs), fs.getCanonicalServiceName(), "canonicalServiceName is not the default");
...
,         credentials.numberOfTokens(), "Number of collected tokens");
  ...
  public StubAbfsTokenIdentifier verifyCredentialsContainsToken(
    ...
    assertEquals(
       StubAbfsTokenIdentifier.TOKEN_KIND, token.getKind(), "Token Kind in " + token);
  ...
  protected String dtutil(final int expected,
    ...
    LOG.info("\n{}", s);
    assertEquals(
       expected, r, "Exit code from command dtutil "
