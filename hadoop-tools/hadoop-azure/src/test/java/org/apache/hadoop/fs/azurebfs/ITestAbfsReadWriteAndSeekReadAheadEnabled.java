...
public class ITestAbfsReadWriteAndSeekReadAheadEnabled extends AbstractAbfsScaleTest {
  ...
  public ITestAbfsReadWriteAndSeekReadAheadEnabled(final int size) throws Exception {
    this.size = size;
  }
  ...
  private void testReadWriteAndSeek(int bufferSize) throws Exception {
    ...
    try (FSDataInputStream inputStreamForReadAhead = fs.open(TEST_PATH)) {
      ...
      assertArrayEquals(readBufferForReadAhead, Arrays.copyOfRange(b, bufferSize, bufferSize));
      inputStreamForReadAhead.seek(0);
      resultForReadAhead = inputStreamForReadAhead.read(readBufferForReadAhead, 0, bufferSize);
    }
    // checking for correctness
    assertNotEquals("data read in final read()", -1, resultForReadAhead);
    assertArrayEquals(readBufferForReadAhead, b);
    }
}
