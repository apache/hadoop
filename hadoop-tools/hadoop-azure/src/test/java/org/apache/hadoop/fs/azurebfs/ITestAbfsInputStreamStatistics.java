...
,           stats.getSeekOperations(), "Mismatch in seekOps value");
      assertEquals(OPERATIONS
,           stats.getBackwardSeekOperations(), "Mismatch in backwardSeekOps value");
      assertEquals(OPERATIONS
,           stats.getForwardSeekOperations(), "Mismatch in forwardSeekOps value");
      assertEquals(
         OPERATIONS * ONE_MB, stats.getBytesBackwardsOnSeek(), "Mismatch in bytesBackwardsOnSeek value");
      assertEquals(
         0, stats.getBytesSkippedOnSeek(), "Mismatch in bytesSkippedOnSeek value");
      assertEquals(OPERATIONS
,           stats.getSeekInBuffer(), "Mismatch in seekInBuffer value");
  ...
  public void testReadStatistics() throws IOException {
    ...
    try {
       ...
       */
      assertEquals(OPERATIONS
,           stats.getBytesRead(), "Mismatch in bytesRead value");
      assertEquals(OPERATIONS
,           stats.getReadOperations(), "Mismatch in readOps value");
      assertEquals(1
,           stats.getRemoteReadOperations(), "Mismatch in remoteReadOps value");
  ...
  public void testWithNullStreamStatistics() throws IOException {
    ...
    try {
      ...
      assertNotEquals(-1, in.read(), "AbfsInputStream read() with null statistics should "
          + "work");
      in.seek(ONE_KB);
