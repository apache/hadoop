...
public class ITestAbfsIdentityTransformer extends AbstractAbfsScaleTest{
  ...
  public void testDaemonServiceSettingIdentity() throws IOException {
    ...
    assertEquals(
           DAEMON, identityTransformer.transformUserOrGroupForSetRequest(DAEMON), "Identity should not change for default config");
    ...
    assertEquals(
           DAEMON, identityTransformer.transformUserOrGroupForSetRequest(DAEMON), "Identity should not change when substitution list doesn't contain daemon");

    // case 2: substitution list contains daemon name
    config.set(FS_AZURE_OVERRIDE_OWNER_SP_LIST, DAEMON + ",a,b,c,d");
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           SERVICE_PRINCIPAL_ID, identityTransformer.transformUserOrGroupForSetRequest(DAEMON), "Identity should be replaced to servicePrincipalId");
  ...
  public void testFullyQualifiedNameSettingIdentity() throws IOException {
    ...
    IdentityTransformer identityTransformer = getTransformerWithDefaultIdentityConfig(config);
    assertEquals(
           SHORT_NAME, identityTransformer.transformUserOrGroupForSetRequest(SHORT_NAME), "short name should not be converted to full name by default");
    ...
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           FULLY_QUALIFIED_NAME, identityTransformer.transformUserOrGroupForSetRequest(SHORT_NAME), "short name should be converted to full name");
  }
  ...
  public void testNoOpForSettingOidAsIdentity() throws IOException {
    ...
    assertEquals(
           principalId, identityTransformer.transformUserOrGroupForSetRequest(principalId), "Identity should not be changed when owner is already a principal id ");
  }
  ...
  public void testNoOpWhenSettingSuperUserAsdentity() throws IOException {
    ...
    IdentityTransformer identityTransformer = getTransformerWithDefaultIdentityConfig(config);
    assertEquals(
           SUPER_USER, identityTransformer.transformUserOrGroupForSetRequest(SUPER_USER), "Identity should not be changed because it is not in substitution list");
  }
  ...
  public void testIdentityReplacementForSuperUserGetRequest() throws IOException {
    ...
    IdentityTransformer identityTransformer = getTransformerWithDefaultIdentityConfig(config);
    assertEquals(
           localUser, identityTransformer.transformIdentityForGetRequest(SUPER_USER, true, localUser), "$superuser should be replaced with local user by default");
  ...
  public void testIdentityReplacementForDaemonServiceGetRequest() throws IOException {
    ...
    IdentityTransformer identityTransformer = getTransformerWithDefaultIdentityConfig(config);
    assertEquals(
           SERVICE_PRINCIPAL_ID, identityTransformer.transformIdentityForGetRequest(SERVICE_PRINCIPAL_ID, true, localUser), "By default servicePrincipalId should not be converted for GetFileStatus(), listFileStatus(), getAcl()");
    ...
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           SERVICE_PRINCIPAL_ID, identityTransformer.transformIdentityForGetRequest(SERVICE_PRINCIPAL_ID, true, localUser), "servicePrincipalId should not be replaced if local daemon user is not in substitution list");
    ...
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           SERVICE_PRINCIPAL_ID, identityTransformer.transformIdentityForGetRequest(SERVICE_PRINCIPAL_ID, true, localUser), "servicePrincipalId should not be replaced if it is not equal to the SPN set in config");
    ...
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           localUser, identityTransformer.transformIdentityForGetRequest(SERVICE_PRINCIPAL_ID, true, localUser), "servicePrincipalId should be transformed to local use");
    ...
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           localUser, identityTransformer.transformIdentityForGetRequest(SERVICE_PRINCIPAL_ID, true, localUser), "servicePrincipalId should be transformed to local user");
  }
  ...
  public void testIdentityReplacementForKinitUserGetRequest() throws IOException {
    ...
    IdentityTransformer identityTransformer = getTransformerWithDefaultIdentityConfig(config);
    assertEquals(
           FULLY_QUALIFIED_NAME, identityTransformer.transformIdentityForGetRequest(FULLY_QUALIFIED_NAME, true, localUser), "full name should not be transformed if shortname is not enabled");

    // add config to get short name
    config.setBoolean(FS_AZURE_FILE_OWNER_ENABLE_SHORTNAME, true);
    identityTransformer = getTransformerWithCustomizedIdentityConfig(config);
    assertEquals(
           SHORT_NAME, identityTransformer.transformIdentityForGetRequest(FULLY_QUALIFIED_NAME, true, localUser), "should convert the full owner name to shortname ");

    assertEquals(
           FULLY_QUALIFIED_NAME, identityTransformer.transformIdentityForGetRequest(FULLY_QUALIFIED_NAME, false, localGroup), "group name should not be converted to shortname ");
  }
