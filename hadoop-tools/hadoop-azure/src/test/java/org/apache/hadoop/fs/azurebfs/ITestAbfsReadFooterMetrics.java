...
public class ITestAbfsReadFooterMetrics extends AbstractAbfsScaleTest {
  ...
  public void testReadFooterMetricsWithParquetAndNonParquet() throws Exception {
    testReadWriteAndSeek(8 * ONE_MB, 4 * ONE_MB, ONE_KB, 4 * ONE_KB);
  }
  ...
  public void testReadFooterMetrics() throws Exception {
    ...
    AbfsReadFooterMetrics abfsReadFooterMetrics = fs.getAbfsClient().getAbfsCounters().getAbfsReadFooterMetrics();
  ...
  private void testReadWriteAndSeek(int fileSize, int bufferSize, Integer seek1, Integer seek2) throws Exception {
    ...
    assertEquals("$Parquet:$FR=1024.000$SR=4096.000$FL=8388608.000$RL=0.000$NonParquet:$FR=16384.000_16384.000$SR=1.000_16384.000$FL=32768.000$RL=16384.000", readFooterMetric);
    try {
      fs.close();
    } catch (Exception e) {
    }
