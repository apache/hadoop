...
summary() {
  {
    ...
    echo "$combination"
    echo "========================"
    pcre2grep -M "$testresultsregex" "$testlogfilename"
...
checkdependencies() {
  if ! [ "$(command -v pcre2grep)" ]; then
    echo "Exiting. pcre2grep is required to run the script."
    echo "Ubuntu: sudo apt install pcre2-utils"
    echo "Centos: sudo yum install pcre2-tools"
    exit 1
  fi
